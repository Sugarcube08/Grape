...
pub fn reference_algorithm_definitions() -> Vec<AlgorithmDefinitionRecord> {
    vec![
        AlgorithmDefinitionRecord {
            ...
            output_schema: "grape.hrv-reference-output.v1".to_string(),
        ...
        AlgorithmDefinitionRecord {
            ...
            output_schema: "grape.sleep-actigraphy-reference-output.v1".to_string(),
            input_requirements_json: json!({
        ...
        AlgorithmDefinitionRecord {
            ...
            output_schema: "grape.edwards-zone-load-reference-output.v1".to_string(),
            input_requirements_json: json!({
        ...
        AlgorithmDefinitionRecord {
            ...
            output_schema: "grape.stress-hrv-hr-reference-output.v1".to_string(),
            input_requirements_json: json!({
...
fn reference_run_record<T: Serialize>(
    ...
    let output_json = serde_json::to_string(&result.output).map_err(|error| {
        GrapeError::message(format!("cannot serialize {label} output: {error}"))
    })?;
    let quality_flags_json = serde_json::to_string(&result.quality_flags).map_err(|error| {
        GrapeError::message(format!("cannot serialize {label} quality flags: {error}"))
    })?;
