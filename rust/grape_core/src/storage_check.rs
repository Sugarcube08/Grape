...
use crate::{
    ...
    store::{
        CURRENT_SCHEMA_VERSION, DecodedFrameInput, GrapeStore, RawEvidenceInput, known_tables,
    },
};
...
pub fn check_storage_database(options: StorageCheckOptions<'_>) -> GrapeResult<StorageCheckReport> {
    ...
    Ok(StorageCheckReport {
        schema: "grape.storage-check-report.v1".to_string(),
        generated_by: "grape-storage-check".to_string(),
...
fn check_table(store: &GrapeStore, table: &str, required: &[&str]) -> StorageTableCheck {
    let mut issues = Vec::new();
...
fn run_storage_self_test(store: &GrapeStore) -> StorageSelfTestReport {
    let mut issues = Vec::new();
    let raw = hex::decode(GET_HELLO_FRAME).unwrap_or_default();
    let parsed = match parse_frame_hex(DeviceType::Grape, GET_HELLO_FRAME) {
        Ok(parsed) => parsed,
        Err(error) => {
    ...
    let raw_input = RawEvidenceInput {
        evidence_id: "grape.storage-check.raw",
        source: "grape.storage-check",
        captured_at: "2026-05-28T00:00:00Z",
        device_model: "WHOOP 5.0 Grape",
        payload: &raw,
    ...
    let decoded_inserted = match store.insert_decoded_frame(DecodedFrameInput {
        frame_id: "grape.storage-check.frame",
        evidence_id: "grape.storage-check.raw",
        parsed: &parsed,
        parser_version: "grape-storage-check",
    }) {
        Ok(inserted) => inserted,
        Err(error) => {
    ...
    let query_roundtrip = match store.raw_evidence("grape.storage-check.raw") {
    ...
    let foreign_key_rejected = match store.insert_decoded_frame(DecodedFrameInput {
        frame_id: "grape.storage-check.missing-evidence-frame",
        evidence_id: "grape.storage-check.missing-evidence",
        parsed: &parsed,
        parser_version: "grape-storage-check",
    }) {
        Ok(_) => {
    ...
    for issue in issues {
        if issue.starts_with("expected schema version") {
            actions.push(StorageCheckNextAction {
                ...
                action: "Back up the SQLite store, run Grape migrations with the current Rust core, then rerun storage.check before capture/import/export writes.".to_string(),
            });
            ...
            actions.push(StorageCheckNextAction {
                ...
                action: "Reopen the database through GrapeStore or enable PRAGMA foreign_keys before any write path uses the connection.".to_string(),
            });
            ...
            .map(|issue| {
                if issue == "missing table" {
                    StorageCheckNextAction {
                        ...
                        action: "Run Grape migrations on this SQLite store before trusting app capture, metrics, export, debug, or health-sync paths.".to_string(),
                    }
...
fn required_columns() -> BTreeMap<&'static str, Vec<&'static str>> {
    ...
    columns.insert("grape_schema_migrations", vec!["version", "applied_at"]);
    columns.insert(
        "raw_evidence",
        vec![
            "evidence_id",
