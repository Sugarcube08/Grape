...
pub const PRIVACY_LINT_REPORT_SCHEMA: &str = "grape.privacy-lint-report.v1";
...
pub fn lint_privacy_path(path: &Path) -> GrapeResult<PrivacyLintReport> {
    if !path.exists() {
    ...
    Ok(PrivacyLintReport {
        ...
        generated_by: "grape-privacy-lint".to_string(),
...
fn privacy_lint_action_for_rule(rule: &str) -> &'static str {
    match rule {
        ...
        "private_whoop_api_material" => {
            "Remove private WHOOP API replay material from Grape artifacts; keep official-app traces only as redacted capture fixtures with provenance."
        }
        "direct_identifier" => {
            "Replace direct user/device identifiers with Grape-owned pseudonyms before export and keep any mapping outside shareable artifacts."
        }
        "email" => {
...
fn lint_file_path(
    ...
    if is_zip_path(path) {
        let file = File::open(path).map_err(|source| GrapeError::io(path, source))?;
        let mut archive = ZipArchive::new(file)
            .map_err(|source| GrapeError::message(format!("cannot open zip archive: {source}")))?;
        for index in 0..archive.len() {
            let mut entry = archive.by_index(index).map_err(|source| {
                GrapeError::message(format!("cannot read zip entry {index}: {source}"))
            })?;
            if entry.is_dir() {
                continue;
            }
            ...
            entry.read_to_end(&mut bytes).map_err(|source| {
                GrapeError::message(format!("cannot read zip bytes: {source}"))
            })?;
