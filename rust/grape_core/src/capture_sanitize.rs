...
pub const CAPTURE_SANITIZE_REPORT_SCHEMA: &str = "grape.capture-sanitize-report.v1";
pub const CAPTURE_SANITIZE_MANIFEST_SCHEMA: &str = "grape.capture-sanitize-manifest.v1";
...
pub fn sanitize_capture_path(
...
) -> GrapeResult<CaptureSanitizeReport> {
    if options.salt.trim().is_empty() {
        return Err(GrapeError::message("sanitize salt must not be empty"));
    }
    ...
    if input_path.is_dir() {
        ...
        fs::create_dir_all(output_path).map_err(|source| GrapeError::io(output_path, source))?;
        ...
        if temp_result.written && target_path != output_path {
            fs::rename(&target_path, output_path)
                .map_err(|source| GrapeError::io(output_path, source))?;
        }
    ...
    Ok(CaptureSanitizeReport {
        ...
        generated_by: "grape-capture-sanitize".to_string(),
...
fn sanitize_one_file(
    ...
    output_root: &Path,
    salt: &str,
) -> GrapeResult<SanitizedFileReport> {
    let bytes = fs::read(input_path).map_err(|source| GrapeError::io(input_path, source))?;
    ...
    let text = String::from_utf8(bytes.clone()).map_err(|source| {
        GrapeError::message(format!("{} is not UTF-8: {source}", input_path.display()))
    })?;
        ...
        CaptureFileFormat::Json => match serde_json::from_str::<serde_json::Value>(&text) {
            Ok(mut value) => {
                ...
                serde_json::to_string_pretty(&value).map_err(|source| {
                    GrapeError::message(format!("cannot serialize sanitized JSON: {source}"))
                })? + "\n"
            }
            Err(source) => {
            ...
            Ok(mut value) => {
                *redactions += sanitize_json_value(&mut value, salt);
                output.push(serde_json::to_string(&value).map_err(|source| {
                    GrapeError::message(format!("cannot serialize sanitized JSONL: {source}"))
                })?);
            }
            Err(source) => {
                warnings.push(format!(
...
fn write_sanitize_manifest(
    ...
    let manifest_json = serde_json::to_vec_pretty(&manifest).map_err(|source| {
        GrapeError::message(format!("cannot serialize sanitize manifest: {source}"))
    })?;
    fs::write(output_path.join("sanitize-manifest.json"), manifest_json)
        .map_err(|source| GrapeError::io(output_path.join("sanitize-manifest.json"), source))?;
    Ok(())
}
...
fn collect_files(root: &Path, files: &mut Vec<PathBuf>) -> GrapeResult<()> {
...
fn reject_output_inside_input(input_path: &Path, output_path: &Path) -> GrapeResult<()> {
    let input = input_path
        .canonicalize()
        .map_err(|source| GrapeError::io(input_path, source))?;
