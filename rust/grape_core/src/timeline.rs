...
use crate::{
    ...
    store::{DebugEventRow, DecodedFrameRow, GrapeStore, RawEvidenceRow},
};
...
) -> GrapeResult<Vec<PacketTimelineRow>> {
...
) -> GrapeResult<Vec<ObservabilityTimelineRow>> {
...
fn timeline_row_from_decoded_frame(row: &DecodedFrameRow) -> GrapeResult<PacketTimelineRow> {
...
fn capture_session_ids_from_debug_rows(
        ...
        .filter(|row| row.topic.starts_with("capture.session."))
    {
        let data: serde_json::Value = serde_json::from_str(&row.data_json).map_err(|error| {
            GrapeError::message(format!("{} data_json invalid: {error}", row.sequence))
        })?;
...
fn observability_row_from_debug_event(
    row: &DebugEventRow,
) -> GrapeResult<Option<ObservabilityTimelineRow>> {
    let data: serde_json::Value = serde_json::from_str(&row.data_json).map_err(|error| {
        GrapeError::message(format!("{} data_json invalid: {error}", row.sequence))
    })?;
