...
use crate::{
    ...
    metrics::{
        HrvInput, RecoveryInput, SleepInput, StrainInput, StressInput, grape_hrv_v0,
        grape_recovery_v0, grape_sleep_v0, grape_strain_v0, grape_stress_v0,
    },
    protocol::{DeviceType, FrameAccumulator, build_v5_payload_frame, parse_frame},
    store::GrapeStore,
};

pub const PERF_BUDGET_REPORT_SCHEMA: &str = "grape.perf-budget-report.v1";
...
pub fn run_perf_budget(options: PerfBudgetOptions) -> GrapeResult<PerfBudgetReport> {
    if options.scale == 0 {
        return Err(GrapeError::message("scale must be greater than 0"));
    }
...
pub fn perf_budget_report_from_workloads(
    ...
    let score_workload_ready = perf_workload_ready(&workloads, "grape_score_batch");
    ...
    PerfBudgetReport {
        ...
        generated_by: "grape-perf-budget".to_string(),
        scale,
        pass,
        input_valid,
...
fn parser_workload(scale: usize, budgets: &PerfBudgets) -> PerfWorkloadReport {
    ...
    for frame in &frames {
        ...
        checks += 1;
        match parse_frame(DeviceType::Grape, frame) {
...
fn deframer_workload(scale: usize, budgets: &PerfBudgets) -> PerfWorkloadReport {
    ...
    let started = Instant::now();
    let mut accumulator = FrameAccumulator::new(DeviceType::Grape);
...
fn algorithms_workload(scale: usize, budgets: &PerfBudgets) -> PerfWorkloadReport {
    ...
    for index in 0..scale {
        ...
        output_failures += grape_hrv_v0(&hrv_input).output.is_none() as usize;
        checks += 1;

        output_failures += grape_sleep_v0(&sleep_input(index)).output.is_none() as usize;
        output_failures += grape_strain_v0(&strain_input(index)).output.is_none() as usize;
        output_failures += grape_recovery_v0(&recovery_input(index)).output.is_none() as usize;
        output_failures += grape_stress_v0(&stress_input(index)).output.is_none() as usize;
        checks += 4;
    }

    let duration_ms = duration_ms(started.elapsed());
    finish_workload(WorkloadFinish {
        name: "grape_score_batch",
        cases: scale * 5,
        checks,
        duration_ms,
...
fn export_workload(scale: usize, budgets: &PerfBudgets) -> GrapeResult<PerfWorkloadReport> {
    let workspace = PerfWorkspace::new()?;
    let db_path = workspace.path.join("grape.sqlite");
    let output_dir = workspace.path.join("export.grapebundle");
    let zip_path = workspace.path.join("export.grapebundle.zip");
    ...
    let import_report = import_captured_frame_batch(
        ...
        CapturedFrameBatchOptions {
            parser_version: "grape-core/perf-budget",
        },
    )?;
    ...
    let export_report = export_raw_timeframe(
        ...
        RawExportOptions {
            ...
            app_version: "grape-app/perf-budget",
            core_version: "grape-core/perf-budget",
...
fn perf_budget_action(workload_name: &str, reason: &str) -> &'static str {
    match reason {
        "duration_budget_exceeded" => match workload_name {
            ...
            "grape_score_batch" => {
                "Profile Grape score calculations at this scale, then cache shared feature work or simplify repeated math before raising the mobile budget."
            }
            "raw_export_bundle" => {
        ...
        "memory_budget_exceeded" => match workload_name {
            ...
            "grape_score_batch" => {
                "Reduce temporary vectors in Grape score inputs/outputs or process windows incrementally before raising the mobile memory budget."
            }
            "raw_export_bundle" => {
        ...
        "algorithm_output_failure" => {
            "Fix Grape score input generation or quality gates so generated valid inputs produce score outputs before benchmarking."
        }
        "raw_export_row_mismatch" => {
...
fn directory_size(path: &Path) -> GrapeResult<u64> {
    let mut total = 0u64;
    for entry in fs::read_dir(path).map_err(|source| GrapeError::io(path, source))? {
        let entry = entry.map_err(|source| GrapeError::io(path, source))?;
        let entry_path = entry.path();
        let metadata = entry
            .metadata()
            .map_err(|source| GrapeError::io(&entry_path, source))?;
        if metadata.is_dir() {
...
fn file_size(path: &Path) -> GrapeResult<u64> {
    Ok(fs::metadata(path)
...
impl PerfWorkspace {
    fn new() -> GrapeResult<Self> {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|error| {
                GrapeError::message(format!("system clock before unix epoch: {error}"))
            })?
            .as_nanos();
        let path =
            std::env::temp_dir().join(format!("grape-perf-budget-{}-{unique}", std::process::id()));
        fs::create_dir_all(&path).map_err(|source| GrapeError::io(&path, source))?;
        Ok(Self { path })
    }
}
