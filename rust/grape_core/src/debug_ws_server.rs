...
pub fn serve_debug_ws_once(options: DebugWsServerOptions) -> GrapeResult<DebugWsServeReport> {
    let listener = bind_debug_ws_listener(&options)?;
    serve_debug_ws_listener_once(listener, options)
}

pub fn bind_debug_ws_listener(options: &DebugWsServerOptions) -> GrapeResult<TcpListener> {
    validate_server_options(options)?;
    let mut addresses = (options.bind_host.as_str(), options.port)
        .to_socket_addrs()
        .map_err(|error| {
            GrapeError::message(format!("cannot resolve debug bind address: {error}"))
        })?;
    let address = addresses
        .next()
        .ok_or_else(|| GrapeError::message("debug bind address did not resolve"))?;
    TcpListener::bind(address)
        .map_err(|error| GrapeError::io(format!("{}:{}", options.bind_host, options.port), error))
}

pub fn serve_debug_ws_listener_once(
    listener: TcpListener,
    options: DebugWsServerOptions,
) -> GrapeResult<DebugWsServeReport> {
    validate_server_options(&options)?;
    let bind_url = debug_ws_url(&listener, &options)?;
    let (stream, _) = listener
        .accept()
        .map_err(|error| GrapeError::io(&options.database_path, error))?;
    ...
    let store = GrapeStore::open(&options.database_path)?;
    ...
    loop {
        let rows = store.debug_events_after_sequence(
            ...
            i64::try_from(last_sequence)
                .map_err(|_| GrapeError::message("last_sequence is too large"))?,
            Some(100),
        )?;
        if rows.is_empty() {
        ...
        for row in rows {
            ...
            let event_json = serde_json::to_string(&event).map_err(|error| {
                GrapeError::message(format!("cannot serialize debug event: {error}"))
            })?;
            websocket
                .send(Message::Text(event_json.into()))
                .map_err(|error| {
                    GrapeError::message(format!("cannot write websocket event: {error}"))
                })?;
            events_sent += 1;
...
fn validate_server_options(options: &DebugWsServerOptions) -> GrapeResult<()> {
    if options.session_id.trim().is_empty() {
        return Err(GrapeError::message("session_id is required"));
    }
    if options.token.trim().is_empty() {
        return Err(GrapeError::message("token is required"));
    }
    if options.poll_interval_ms == 0 {
        return Err(GrapeError::message("poll_interval_ms must be positive"));
    }
    if options.idle_timeout_ms == 0 {
        return Err(GrapeError::message("idle_timeout_ms must be positive"));
    }
    let issues = debug_bridge_config_issues(&DebugBridgeConfig {
        url: format!(
            "ws://{}:{}/grape-debug/stream?token={}",
...
fn validate_handshake_request(request: &Request, token: &str) -> Result<(), ErrorResponse> {
    if request.uri().path() != "/grape-debug/stream" {
        return Err(error_response(
            StatusCode::NOT_FOUND,
            "expected /grape-debug/stream",
        ));
    }
...
fn debug_ws_url(listener: &TcpListener, options: &DebugWsServerOptions) -> GrapeResult<String> {
...
fn report(
    ...
    DebugWsServeReport {
        schema: "grape.debug-ws-serve-report.v1".to_string(),
        generated_by: "grape-debug-ws-serve".to_string(),
        bind_url,
        session_id,
...
fn debug_ws_serve_issue_action(issue: &str) -> String {
    if issue.contains("websocket handshake failed") {
        return "Connect with path /grape-debug/stream and the current per-session token."
            .to_string();
    }
    match issue {
