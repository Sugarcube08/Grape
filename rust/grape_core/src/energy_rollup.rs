...
use crate::{
    ...
    store::{
        DailyActivityMetricInput, DailyActivityMetricRow, GrapeStore, HourlyActivityMetricInput,
...
pub const ENERGY_DAILY_ROLLUP_REPORT_SCHEMA: &str = "grape.energy-daily-rollup-report.v1";
pub const ENERGY_HOURLY_ROLLUP_REPORT_SCHEMA: &str = "grape.energy-hourly-rollup-report.v1";
pub const ENERGY_CAPTURE_VALIDATION_REPORT_SCHEMA: &str =
    "grape.energy-capture-validation-report.v1";
pub const ENERGY_UNAVAILABLE_DAILY_STATUS_REPORT_SCHEMA: &str =
    "grape.energy-unavailable-daily-status-report.v1";
pub const GRAPE_ENERGY_LOCAL_ESTIMATE_V0_ID: &str = "grape.energy.local_estimate.v0";
pub const GRAPE_ENERGY_LOCAL_ESTIMATE_V0_VERSION: &str = "0.1.0";
pub const GRAPE_ENERGY_UNAVAILABLE_STATUS_V0_ID: &str = "grape.energy.unavailable_status.v0";
pub const GRAPE_ENERGY_UNAVAILABLE_STATUS_V0_VERSION: &str = "0.1.0";
...
pub fn rollup_energy_day_for_store(
...
) -> GrapeResult<EnergyDailyRollupReport> {
    ...
    if pass && options.write_metric {
        ...
        let quality_flags_json = serde_json::to_string(&quality_flags).map_err(|error| {
            GrapeError::message(format!("cannot serialize energy quality flags: {error}"))
        })?;
        let provenance_json = json!({
            "algorithm": GRAPE_ENERGY_LOCAL_ESTIMATE_V0_ID,
            "algorithm_version": GRAPE_ENERGY_LOCAL_ESTIMATE_V0_VERSION,
    ...
    Ok(EnergyDailyRollupReport {
        ...
        generated_by: "grape-energy-daily-rollup".to_string(),
        pass,
...
pub fn rollup_energy_unavailable_daily_status_for_store(
...
) -> GrapeResult<EnergyUnavailableDailyStatusReport> {
    ...
    Ok(EnergyUnavailableDailyStatusReport {
        ...
        generated_by: "grape-energy-unavailable-daily-status".to_string(),
        pass: true,
...
fn energy_unavailable_metric_status_for_rollup(
...
) -> GrapeResult<EnergyUnavailableMetricStatus> {
    ...
    if write_metric {
        ...
        let provenance_json = json!({
            "algorithm": GRAPE_ENERGY_UNAVAILABLE_STATUS_V0_ID,
            "algorithm_version": GRAPE_ENERGY_UNAVAILABLE_STATUS_V0_VERSION,
...
pub fn rollup_energy_hour_for_store(
...
) -> GrapeResult<EnergyHourlyRollupReport> {
    ...
    Ok(EnergyHourlyRollupReport {
        ...
        generated_by: "grape-energy-hourly-rollup".to_string(),
        pass,
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start: options.start.to_string(),
...
pub fn validate_energy_capture_for_store(
...
) -> GrapeResult<EnergyCaptureValidationReport> {
    ...
    Ok(EnergyCaptureValidationReport {
        ...
        generated_by: "grape-energy-capture-validator".to_string(),
        ...
        algorithm_id: GRAPE_ENERGY_LOCAL_ESTIMATE_V0_ID.to_string(),
        algorithm_version: GRAPE_ENERGY_LOCAL_ESTIMATE_V0_VERSION.to_string(),
        energy_rollup,
...
) -> GrapeResult<StepCadenceSupport> {
...
fn validate_options(options: &EnergyDailyRollupOptions<'_>) -> GrapeResult<()> {
    ...
    if let Some(resting_hr_bpm) = options.resting_hr_bpm {
        if !resting_hr_bpm.is_finite() || resting_hr_bpm <= 0.0 {
            return Err(GrapeError::message("resting_hr_bpm must be positive"));
        }
    }
    if let Some(max_hr_bpm) = options.max_hr_bpm {
        if !max_hr_bpm.is_finite() || max_hr_bpm <= 0.0 {
            return Err(GrapeError::message("max_hr_bpm must be positive"));
        }
    }
...
fn validate_hourly_options(options: &EnergyHourlyRollupOptions<'_>) -> GrapeResult<()> {
    if options.date_key.trim().is_empty() {
        return Err(GrapeError::message("date_key is required"));
    }
    if options.timezone.trim().is_empty() {
...
fn validate_energy_validation_options(
    ...
    if !options.tolerance_kcal.is_finite() || options.tolerance_kcal < 0.0 {
        return Err(GrapeError::message("tolerance_kcal must be nonnegative"));
    }
    ...
    for (name, value) in [
        ...
        if let Some(value) = value {
            if !value.is_finite() || value < 0.0 {
                return Err(GrapeError::message(format!("{name} must be nonnegative")));
            }
        }
    }
    Ok(())
