...
pub const UI_COVERAGE_AUDIT_SCHEMA: &str = "grape.ui-coverage-audit.v1";
...
pub fn run_ui_coverage_audit(
    ...
    base_dir: &Path,
) -> GrapeResult<UiCoverageAuditReport> {
    ...
    Ok(UiCoverageAuditReport {
        schema: "grape.ui-coverage-audit-report.v1".to_string(),
        generated_by: "grape-ui-coverage-audit".to_string(),
        inventory_valid,
        coverage_map_valid,
...
fn record_deferred_surface(
    ...
    bucket.deferred_surfaces.push(DeferredUiSurface {
        ...
        grape_area: grape_area.to_string(),
...
fn ui_coverage_issue_next_action(issue: &str) -> UiCoverageNextAction {
    ...
    let action = match issue_kind {
        "unsupported_schema" => {
            "Update the coverage-map schema to grape.ui-coverage-audit.v1, then rerun UI coverage audit."
        }
        ...
        "navigation_rule_grape_area_required"
        | "layout_rule_grape_area_required"
        | "ui_resource_rule_grape_area_required"
        | "source_class_rule_grape_area_required" => {
            "Set grape_area to the Grape screen or area responsible for this surface, then rerun UI coverage audit."
        }
        ...
        | "source_class_rule_target_level_required" => {
            "Set target_level for the implemented, approximate, or debug-only Grape coverage target, then rerun UI coverage audit."
        }
        ...
        _ if issue_kind.ends_with("_missing_coverage") => {
            "Add or update a coverage-map rule for this APK surface with a status, Grape area, target level or reason, then rerun UI coverage audit."
        }
        _ => {
...
fn validate_decision(
    ...
    if grape_area.trim().is_empty() {
        issues.push(format!("{surface_kind}_rule_grape_area_required:{rule_id}"));
    }
...
fn file_sha256(path: &Path) -> GrapeResult<String> {
