...
fn partial_plan_next_actions(scope: &str, state: PartialPlanState) -> Vec<HealthSyncNextAction> {
    if state.confirmation_required {
        return vec![HealthSyncNextAction {
            ...
            action: "Ask the user to confirm syncing only the planned Grape rows; blocked rows must remain unwritten.".to_string(),
        }];
    }
...
pub fn run_health_sync_dry_run(input: &HealthSyncDryRunInput) -> HealthSyncDryRunReport {
    ...
    if input.schema != "grape.health-sync-dry-run.v1" {
    ...
    for candidate in &input.candidates {
        ...
        if reasons.is_empty() {
            ...
            planned_writes.push(PlannedHealthWrite {
                ...
                idempotency_key,
                grape_marker: grape_marker(candidate),
    ...
    HealthSyncDryRunReport {
        schema: "grape.health-sync-dry-run-report.v1".to_string(),
        generated_by: "grape-health-sync-dry-run".to_string(),
...
pub fn run_activity_health_sync_dry_run(
    ...
    let mut issues = Vec::new();
    if input.schema != "grape.activity-health-sync-dry-run.v1" {
        issues.push(format!("unsupported schema {}", input.schema));
    }
    let backfill_times = validate_backfill_window(&input.backfill, &mut issues);
        ...
        if reasons.is_empty() {
            ...
            if !seen_keys.insert(idempotency_key.clone()) {
                ...
                planned_sessions.push(PlannedActivityHealthWrite {
                    ...
                    idempotency_key,
                    grape_marker: activity_grape_marker(session),
    ...
    next_actions = dedupe_health_next_actions(next_actions);
    ActivityHealthSyncDryRunReport {
        schema: "grape.activity-health-sync-dry-run-report.v1".to_string(),
        generated_by: "grape-activity-health-sync-dry-run".to_string(),
        platform: input.platform,
        pass: input_valid,
        input_valid,
        partial_plan_policy: input.partial_plan_policy,
...
fn activity_grape_marker(session: &ActivitySyncCandidate) -> String {
    format!(
        "grape:{}:{}:{}",
    ...
    for existing in &input.existing_records {
        ...
        if reasons.is_empty() {
            planned_deletes.push(PlannedHealthDelete {
                ...
                grape_marker: existing.grape_marker.clone(),
                start_time: existing.start_time.clone(),
                end_time: existing.end_time.clone(),
                reason: "stale_grape_record_in_backfill".to_string(),
            });
        } else {
            reasons.sort();
            reasons.dedup();
    ...
    !blocked_deletes
        ...
        .any(|reason| {
            reason.ends_with("_required")
                || matches!(
                    reason.as_str(),
                    "outside_backfill_window" | "end_time_not_after_start_time" | "not_grape_owned"
                )
        })
}
...
fn validate_existing_record_for_delete(
    ...
    for (name, value) in [
        ...
        ("grape_marker", existing.grape_marker.as_str()),
    ...
    if !existing.grape_marker.starts_with("grape:")
        || !existing.idempotency_key.starts_with("grape:")
    {
        reasons.push("not_grape_owned".to_string());
    }
...
fn health_sync_issue_action(issue: &str) -> String {
    if issue.starts_with("unsupported schema ") {
        return "Use grape.health-sync-dry-run.v1 input before planning Health sync.".to_string();
    }
...
fn health_sync_reason_action(
    ...
    match reason {
        ...
        "unsupported_delete_mapping" => {
            "Leave this platform record untouched or add a supported Grape cleanup mapping."
                .to_string()
        }
        ...
        "unsafe_source_kind" => {
            "Use a Grape-owned decoded, derived, user-confirmed, or manual source kind."
                .to_string()
        }
        "unsupported_activity_type_mapping" => {
            "Use a supported Grape activity, workout, or sleep type before planning this session."
                .to_string()
        }
        ...
        "benchmark_only_algorithm_not_syncable" => {
            "Use a Grape-owned primary algorithm output instead of a benchmark/reference output."
                .to_string()
        }
        "missing_provenance" => {
            "Attach non-empty Grape provenance linking this value to owned data or an approved algorithm run."
                .to_string()
        }
        "provenance_must_be_object" => {
        ...
        "private_api_provenance_not_syncable" => {
            "Replace private API provenance with user-owned capture/import provenance; Grape must not sync private API replay material."
                .to_string()
        }
        "official_whoop_label_not_syncable" => {
            "Keep official WHOOP labels as labels only; sync Grape outputs or decoded owned values instead."
                .to_string()
        }
        "platform_import_not_syncable" => {
            "Keep HealthKit and Health Connect values out of Grape local metrics; only sync Grape-owned decoded or derived outputs."
                .to_string()
        }
        "healthkit_rmssd_must_not_be_written_as_sdnn" => {
            "Do not write RMSSD to HealthKit SDNN; sync SDNN only if Grape can calculate SDNN, or use Health Connect RMSSD."
                .to_string()
        }
        "health_connect_has_no_sdnn_record" => {
        ...
        "not_grape_owned" => {
            "Do not delete external platform records; cleanup is limited to Grape-owned records."
                .to_string()
        }
...
fn idempotency_key(
    ...
    format!(
        "grape:{platform:?}:{destination_type}:{}:{}:{}",
        candidate.record_id, candidate.start_time, candidate.end_time
    )
}

fn grape_marker(candidate: &HealthSyncCandidate) -> String {
    format!(
        "grape:{}:{}:{}",
