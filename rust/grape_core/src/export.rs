...
use crate::{
    ...
    store::{
        ...
        DebugSessionRow, DecodedFrameRow, GrapeStore, HourlyActivityMetricRow, MetricProvenanceRow,
        RawEvidenceRow,
    },
...
pub fn validate_export_bundle(path: &Path) -> GrapeResult<ExportValidationReport> {
    if path
        .extension()
    ...
    let manifest_raw = fs::read_to_string(&manifest_path)
        .map_err(|source| GrapeError::io(&manifest_path, source))?;
    let manifest: ExportManifest = serde_json::from_str(&manifest_raw)
        .map_err(|source| GrapeError::json(&manifest_path, source))?;
    ...
    let content = validate_export_contents(&manifest, |relative_path| {
        fs::read_to_string(base_dir.join(relative_path))
            .map_err(|source| GrapeError::io(base_dir.join(relative_path), source))
    });
...
pub fn export_raw_timeframe(
    ...
    fs::create_dir_all(options.output_dir)
        .map_err(|source| GrapeError::io(options.output_dir, source))?;
    fs::create_dir_all(options.output_dir.join("data"))
        .map_err(|source| GrapeError::io(options.output_dir.join("data"), source))?;
    ...
    if raw_export_family_selected(&selected_data_families, RAW_EXPORT_SQLITE_FAMILY) {
        let sqlite_source_path = options
            .sqlite_source_path
            .ok_or_else(|| GrapeError::message("sqlite data family requires sqlite_source_path"))?;
        let sqlite_target_path = options.output_dir.join("data/grape.sqlite");
        snapshot_sqlite_database(sqlite_source_path, &sqlite_target_path)?;
        let bytes = fs::read(&sqlite_target_path)
            .map_err(|source| GrapeError::io(&sqlite_target_path, source))?;
        manifest_files.push(manifest_file("data/grape.sqlite", &bytes, None, "sqlite"));
    }

    let manifest = ExportManifest {
        schema_version: "grape.export.v1".to_string(),
    ...
    let manifest_json = serde_json::to_vec_pretty(&manifest)
        .map_err(|error| GrapeError::message(format!("cannot serialize manifest: {error}")))?;
    fs::write(options.output_dir.join("manifest.json"), manifest_json)
        .map_err(|source| GrapeError::io(options.output_dir.join("manifest.json"), source))?;
    ...
    Ok(RawExportReport {
        schema: "grape.raw-export-report.v1".to_string(),
        generated_by: "grape-raw-export".to_string(),
    ...
    for file in &manifest.files {
        ...
        match export_manifest_file_family(&file.path) {
            ...
            None => issues.push(format!(
                "{} is not a recognized Grape export artifact path",
                file.path
            )),
        }
    }
...
fn snapshot_sqlite_database(source_path: &Path, target_path: &Path) -> GrapeResult<()> {
    fs::metadata(source_path).map_err(|source| GrapeError::io(source_path, source))?;
    if target_path.exists() {
        fs::remove_file(target_path).map_err(|source| GrapeError::io(target_path, source))?;
    }
...
fn raw_evidence_rows_between_for_raw_export(
    ...
    filters: &RawExportFilters,
) -> GrapeResult<Vec<RawEvidenceRow>> {
...
fn debug_commands_for_raw_byte_policy(
    ...
    filters: &RawExportFilters,
) -> GrapeResult<Vec<ExportDebugCommandRow>> {
    if filters.include_raw_bytes {
        return Ok(rows.to_vec());
    }
    rows.iter()
...
fn debug_events_for_raw_byte_policy(
    ...
    filters: &RawExportFilters,
) -> GrapeResult<Vec<ExportDebugEventRow>> {
    if filters.include_raw_bytes {
        return Ok(rows.to_vec());
    }
    rows.iter()
...
fn raw_byte_policy_json_text(text: &str) -> GrapeResult<String> {
    let value: Value = serde_json::from_str(text).map_err(|error| {
        GrapeError::message(format!("cannot parse JSON for raw-byte policy: {error}"))
    })?;
    serde_json::to_string(&raw_byte_policy_json_value(value)).map_err(|error| {
        GrapeError::message(format!("cannot serialize raw-byte policy JSON: {error}"))
    })
}

fn raw_byte_policy_json_text_if_valid(text: &str) -> GrapeResult<String> {
...
fn write_jsonl<T: Serialize>(path: &Path, rows: &[T]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    for row in rows {
        serde_json::to_writer(&mut bytes, row)
            .map_err(|error| GrapeError::message(format!("cannot serialize JSONL row: {error}")))?;
        bytes.push(b'\n');
    }
    fs::write(path, &bytes).map_err(|source| GrapeError::io(path, source))?;
    Ok(bytes)
}

fn write_raw_csv(path: &Path, rows: &[RawEvidenceRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
        "evidence_id,source,captured_at,device_model,payload_hex,sha256,sensitivity,capture_session_id"
    )
    .map_err(|error| GrapeError::message(format!("cannot write CSV header: {error}")))?;
    for row in rows {
        write_csv_row(
            &mut bytes,
            &[
...
fn write_decoded_csv(path: &Path, rows: &[DecodedFrameRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_packet_timeline_csv(path: &Path, rows: &[PacketTimelineRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_sensor_samples_csv(path: &Path, rows: &[ExportSensorSampleRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
) -> GrapeResult<Vec<ExportSensorSampleRow>> {
...
) -> GrapeResult<Vec<ExportMetricFeatureReportRow>> {
    let mut rows = Vec::new();
...
fn push_metric_feature_report(
    ...
    let report_json = serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize metric feature report: {error}"))
    })?;
    ...
    let issues_json = serde_json::to_string(&issues_json)
        .map_err(|error| GrapeError::message(format!("cannot serialize report issues: {error}")))?;
...
fn write_metric_values_csv(path: &Path, rows: &[ExportMetricValueRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
) -> GrapeResult<(Vec<ExportMetricValueRow>, Vec<ExportMetricComponentRow>)> {
...
fn parse_quality_flags_for_metric_output(run: &AlgorithmRunRecord) -> GrapeResult<Vec<String>> {
...
fn write_algorithm_runs_csv(path: &Path, rows: &[AlgorithmRunRecord]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_activity_sessions_csv(path: &Path, rows: &[ActivitySessionRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_activity_metrics_csv(path: &Path, rows: &[ActivityMetricRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_activity_intervals_csv(path: &Path, rows: &[ActivityIntervalRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_activity_labels_csv(path: &Path, rows: &[ActivityLabelRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_metric_provenance_csv(path: &Path, rows: &[MetricProvenanceRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_calibration_runs_csv(path: &Path, rows: &[CalibrationRunRecord]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_debug_sessions_csv(path: &Path, rows: &[ExportDebugSessionRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_debug_commands_csv(path: &Path, rows: &[ExportDebugCommandRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_debug_events_csv(path: &Path, rows: &[ExportDebugEventRow]) -> GrapeResult<Vec<u8>> {
    let mut bytes = Vec::new();
    writeln!(
        bytes,
...
fn write_csv_row(output: &mut Vec<u8>, fields: &[&str]) -> GrapeResult<()> {
    for (index, field) in fields.iter().enumerate() {
        if index > 0 {
            output.push(b',');
        }
        write!(output, "{}", csv_escape(field))
            .map_err(|error| GrapeError::message(format!("cannot write CSV field: {error}")))?;
    }
    output.push(b'\n');
    Ok(())
}
...
) -> GrapeResult<Vec<ExportCommandValidationRow>> {
    rows.into_iter()
        .map(|row| {
...
fn validate_zipped_export_bundle(path: &Path) -> GrapeResult<ExportValidationReport> {
    let zip_file = File::open(path).map_err(|source| GrapeError::io(path, source))?;
    let mut archive = ZipArchive::new(zip_file)
        .map_err(|error| GrapeError::message(format!("cannot open zip bundle: {error}")))?;
    let manifest_raw = read_zip_entry_to_string(&mut archive, "manifest.json")?;
    let manifest: ExportManifest =
        serde_json::from_str(&manifest_raw).map_err(|source| GrapeError::json(path, source))?;
...
fn validate_export_contents(
    manifest: &ExportManifest,
    mut read_file: impl FnMut(&str) -> GrapeResult<String>,
...
fn read_typed_export_rows<T: DeserializeOwned>(
    read_file: &mut impl FnMut(&str) -> GrapeResult<String>,
    manifest: &ExportManifest,
    family: &str,
    path: &str,
    issues: &mut Vec<String>,
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        match serde_json::from_str(line) {
            ...
            Err(error) => issues.push(format!(
                "{path} line {} cannot be re-imported as Grape data: {error}",
                index + 1
            )),
        }
    }
...
fn validate_debug_command_reimport(
    ...
    for row in rows {
        ...
        if row.schema != "grape.debug.command.v1" {
            issues.push(format!(
                "debug command {} schema must be grape.debug.command.v1",
                row.command_id
            ));
        }
        if let Some(session_ids) = session_ids {
...
fn validate_debug_event_reimport(
    ...
    for row in rows {
        ...
        if row.schema != "grape.debug.event.v1" {
            issues.push(format!(
                "debug event {}:{} schema must be grape.debug.event.v1",
                row.session_id, row.sequence
            ));
        }
        if let Some(session_ids) = session_ids {
...
fn export_manifest_file_family(path: &str) -> Option<&'static str> {
    match path {
        ...
        "data/grape.sqlite" => Some(RAW_EXPORT_SQLITE_FAMILY),
        _ => None,
    }
}
...
fn required_export_artifact_paths_for_family(family: &str) -> &'static [&'static str] {
    match family {
        ...
        RAW_EXPORT_SQLITE_FAMILY => &["data/grape.sqlite"],
        _ => &[],
    }
}
...
fn write_export_zip(
    ...
    let zip_file =
        File::create(zip_output_path).map_err(|source| GrapeError::io(zip_output_path, source))?;
    ...
    writer
        .finish()
        .map_err(|error| GrapeError::message(format!("cannot finish zip bundle: {error}")))?;
    Ok(())
}
...
fn write_zip_file(
    ...
    let bytes = fs::read(source_path).map_err(|source| GrapeError::io(source_path, source))?;
    writer.start_file(archive_path, options).map_err(|error| {
        GrapeError::message(format!("cannot add {archive_path} to zip: {error}"))
    })?;
    writer.write_all(&bytes).map_err(|error| {
        GrapeError::message(format!("cannot write {archive_path} to zip: {error}"))
    })?;
    Ok(())
}
...
fn read_zip_entry_to_string(archive: &mut ZipArchive<File>, path: &str) -> GrapeResult<String> {
    let bytes = read_zip_entry(archive, path)?;
    String::from_utf8(bytes)
        .map_err(|error| GrapeError::message(format!("{path} is not valid UTF-8: {error}")))
}

fn read_zip_entry(archive: &mut ZipArchive<File>, path: &str) -> GrapeResult<Vec<u8>> {
    validate_safe_relative_path(path)?;
    let mut entry = archive
        .by_name(path)
        .map_err(|error| GrapeError::message(format!("{path}: {error}")))?;
    let mut bytes = Vec::new();
    entry
        .read_to_end(&mut bytes)
        .map_err(|error| GrapeError::message(format!("cannot read {path}: {error}")))?;
    Ok(bytes)
}

fn validate_safe_relative_path(path: &str) -> GrapeResult<()> {
...
fn export_validation_issue_action(default_scope: &str, issue: &str) -> ExportValidationNextAction {
    ...
    } else if issue_lower.contains("cannot be re-imported as grape data") {
        (
            "typed_reimport_failed",
            "Regenerate the typed export row from Grape SQLite or update the typed parser/schema before revalidating.",
        )
        ...
        .contains("sqlite data family cannot be exported when include_raw_bytes is false")
    {
        ...
        (
            "sqlite_source_path",
            "Provide the Grape SQLite source path or deselect the sqlite family before exporting.",
        )
        ...
        || issue_lower.contains("duplicate family")
    {
        (
            ...
            "Regenerate the export manifest with only supported Grape data families selected once.",
        )
    } else if issue_lower.contains("unselected data family") {
        (
            "unselected_data_family_artifact",
            "Regenerate the export so files and row counts exist only for selected Grape data families.",
        )
    } else if issue_lower.contains("not a recognized grape export artifact path") {
        (
            "unknown_manifest_file",
            "Regenerate the export so the manifest lists only canonical Grape artifact paths.",
        )
        ...
        || issue_lower.contains("official labels")
    {
        (
            ...
            "Mark official WHOOP comparison values as labels, not Grape outputs, and re-export calibration labels.",
        )
        ...
        || issue_lower.contains("duplicate ")
    {
        ...
        (
            "invalid_hex",
            "Regenerate the row with valid hex and checksum fields from Grape storage.",
        )
    } else if issue_lower.contains("is required for re-import") {
        (
            "required_field",
            "Regenerate the export from normalized Grape rows so required fields are populated.",
        )
        ...
        || issue_lower.contains("json must be")
    {
        (
            "json_field",
            "Regenerate the row with valid JSON field content from Grape storage.",
        )
        ...
        || issue_lower.contains("activity label")
    {
        (
            ...
            "Regenerate activity session, metric, interval, and label rows from Grape storage so timestamps, enums, linkage, and provenance fields stay consistent.",
        )
        ...
        || issue_lower.contains("debug event")
    {
        ...
        (
            ...
            "Regenerate command-validation rows from Grape storage after importing trusted official-app command evidence.",
        )
...
fn report(
    ...
    ExportValidationReport {
        schema: "grape.export-validation-report.v1".to_string(),
        generated_by: "grape-export-validator".to_string(),
