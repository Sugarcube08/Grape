...
fn run() -> grape_core::GrapeResult<()> {
    let args = args();
    let database_path = default_path(&args, "--db", "grape.sqlite")?;
    let session_id = value(&args, "--session-id")?
        .ok_or_else(|| grape_core::GrapeError::message("--session-id is required"))?;
    let token = value(&args, "--token")?
        .ok_or_else(|| grape_core::GrapeError::message("--token is required"))?;
...
fn parse_u16(value: &str, name: &str) -> grape_core::GrapeResult<u16> {
    value
        .parse::<u16>()
        .map_err(|error| grape_core::GrapeError::message(format!("{name} must be a u16: {error}")))
}

fn parse_u64(value: &str, name: &str) -> grape_core::GrapeResult<u64> {
    value
        .parse::<u64>()
        .map_err(|error| grape_core::GrapeError::message(format!("{name} must be a u64: {error}")))
}

fn parse_usize(value: &str, name: &str) -> grape_core::GrapeResult<usize> {
    value.parse::<usize>().map_err(|error| {
        grape_core::GrapeError::message(format!("{name} must be a usize: {error}"))
    })
}
