...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let json = fs::read_to_string(&evidence_path)
        .map_err(|source| GrapeError::io(&evidence_path, source))?;
    let input = serde_json::from_str::<HistoricalSyncPhysicalValidationInput>(&json)
        .map_err(|error| GrapeError::message(format!("invalid physical evidence JSON: {error}")))?;
...
fn generation_arg(args: &[String]) -> grape_core::GrapeResult<HistoricalSyncGeneration> {
        ...
        .as_str()
    {
        ...
        value => Err(GrapeError::message(format!(
