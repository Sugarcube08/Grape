...
use grape_core::{
    ...
    store::{AlgorithmDefinitionRecord, AlgorithmRunRecord, GrapeStore},
...
const EXTERNAL_REFERENCE_OUTPUT_SCHEMA: &str = "grape.external-reference-output.v1";
...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let report = ReferenceAlgoReport {
        schema: "grape.reference-algo-report.v1".to_string(),
        generated_by: "grape-reference-algo-runner".to_string(),
        family: run.family,
        provider,
...
) -> grape_core::GrapeResult<ReferenceRun> {
...
fn run_typed<I, O>(
    ...
    record: impl FnOnce(&str, &AlgorithmRunResult<O>) -> grape_core::GrapeResult<AlgorithmRunRecord>,
) -> grape_core::GrapeResult<ReferenceRun>
where
    I: DeserializeOwned,
    O: Serialize,
...
fn run_external_reference_algorithm(
    ...
    let input_bytes = fs::read(input_path).map_err(|source| GrapeError::io(input_path, source))?;
    ...
    command
        ...
        .env("GRAPE_REFERENCE_INPUT_PATH", input_path)
        .env("GRAPE_REFERENCE_INPUT_SHA256", &input_sha256)
        .env("GRAPE_REFERENCE_FAMILY", family)
        .env("GRAPE_REFERENCE_PROVIDER", provider)
        .env(
            "GRAPE_REFERENCE_OUTPUT_SCHEMA",
...
fn reference_next_actions(
    ...
    if !storage_ready {
        actions.push(ReferenceNextAction {
            ...
            action: "Rerun with a writable Grape SQLite database or omit --db for a report-only benchmark.".to_string(),
        });
    }
    for error in errors {
        actions.push(ReferenceNextAction {
...
fn external_reference_definition(
    report: &ExternalReferenceOutput,
) -> grape_core::GrapeResult<AlgorithmDefinitionRecord> {
    Ok(AlgorithmDefinitionRecord {
        ...
        input_schema: report
            .input_schema
            .clone()
            .unwrap_or_else(|| format!("grape.{}-input.v1", report.family)),
...
fn values(args: &[String], name: &str) -> grape_core::GrapeResult<Vec<String>> {
    let mut values = Vec::new();
    let mut iter = args.iter();
    while let Some(arg) = iter.next() {
        if arg == name {
            let Some(value) = iter.next() else {
                return Err(GrapeError::message(format!("missing value for {name}")));
            };
...
fn require_non_empty(field: &str, value: &str) -> grape_core::GrapeResult<()> {
...
fn require_object(field: &str, value: &serde_json::Value) -> grape_core::GrapeResult<()> {
    if !value.is_object() {
        return Err(GrapeError::message(format!(
...
fn require_array(field: &str, value: &serde_json::Value) -> grape_core::GrapeResult<()> {
    if !value.is_array() {
        return Err(GrapeError::message(format!(
...
fn reference_input_path(
    args: &[String],
    family: &str,
) -> grape_core::GrapeResult<std::path::PathBuf> {
    match family {
        "hrv" => default_path(
            args,
            "--input",
        ...
        "stress" => default_path(
            args,
            "--input",
            "fixtures/synthetic/stress_grape_v0_hand_derived.json",
        ),
