...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let output_dir = default_path(&args, "--output-dir", "exports/latest.grapebundle")?;
    ...
    let app_version = value(&args, "--app-version")?.unwrap_or_else(|| "grape-app/dev".to_string());
...
fn csv_values(args: &[String], key: &str) -> grape_core::GrapeResult<Vec<String>> {
    Ok(value(args, key)?
        .map(|values| {
            values
                .split(',')
