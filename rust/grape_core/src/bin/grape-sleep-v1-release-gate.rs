...
fn release_gate_input_from_report_paths(
    args: &[String],
) -> grape_core::GrapeResult<SleepV1ReleaseGateInput> {
...
fn optional_json<T: serde::de::DeserializeOwned>(
    args: &[String],
    name: &str,
) -> grape_core::GrapeResult<Option<T>> {
    path_value(args, name)?.map_or(Ok(None), |path| read_json(&path).map(Some))
}

fn benchmark_reports(args: &[String]) -> grape_core::GrapeResult<Vec<AlgorithmComparisonReport>> {
...
fn optional_usize(args: &[String], name: &str) -> grape_core::GrapeResult<Option<usize>> {
