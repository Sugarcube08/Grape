...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let Some(database_path) = value(&args, "--db")? else {
        return Err(GrapeError::message("missing --db <grape.sqlite>"));
    };
    let Some(input_path) = path_value(&args, "--input")? else {
        return Err(GrapeError::message(
            "missing --input <grape.sleep-v1-input.json>",
        ));
    };
    ...
    let store = GrapeStore::open(Path::new(&database_path))?;
...
fn optional_f64(args: &[String], name: &str) -> grape_core::GrapeResult<Option<f64>> {
    value(args, name)?.map_or(Ok(None), |raw| {
        raw.parse::<f64>()
            .map(Some)
            .map_err(|error| GrapeError::message(format!("invalid {name} value {raw}: {error}")))
    })
}

fn read_json<T: serde::de::DeserializeOwned>(path: &Path) -> grape_core::GrapeResult<T> {
