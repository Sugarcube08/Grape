...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let method = value(&args, "--method")?
        ...
        .ok_or_else(|| GrapeError::message("--method is required"))?;
    let method = metric_bridge_method(&method)?;
    let database_path = path_value(&args, "--database")?
        .ok_or_else(|| GrapeError::message("--database is required"))?;
    ...
    if !response.ok {
        ...
        return Err(GrapeError::message(message));
    }
    let report = response
        .result
        .ok_or_else(|| GrapeError::message("metric feature report missing result"))?;
...
fn metric_bridge_method(value: &str) -> grape_core::GrapeResult<&'static str> {
    match value.trim() {
...
fn insert_usize_arg(
    ...
    if let Some(raw) = value(args, name)? {
        let parsed = raw
            .parse::<usize>()
            .map_err(|source| GrapeError::message(format!("invalid {name}: {source}")))?;
...
fn merge_args_json(
    ...
    if let Some(raw) = value(args, "--args-json")? {
        merge_args_value(
            ...
            serde_json::from_str(&raw).map_err(|source| {
                GrapeError::message(format!("invalid --args-json object: {source}"))
            })?,
        )?;
    }
    if let Some(path) = path_value(args, "--args-json-file")? {
        ...
        merge_args_value(
            object,
            serde_json::from_str(&raw).map_err(|source| {
                GrapeError::message(format!("invalid --args-json-file object: {source}"))
            })?,
        )?;
    }
    Ok(())
...
fn merge_args_value(object: &mut Map<String, Value>, value: Value) -> grape_core::GrapeResult<()> {
