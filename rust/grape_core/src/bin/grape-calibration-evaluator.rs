...
use grape_core::{
    ...
    store::{AlgorithmDefinitionRecord, GrapeStore},
...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let algorithm_id =
        value(&args, "--algorithm-id")?.unwrap_or_else(|| "grape.recovery.v0".to_string());
        ...
        && let Some(db_path) = db.as_deref()
    {
        ...
        store.upsert_algorithm_definition(&AlgorithmDefinitionRecord {
            ...
            input_schema: "grape.calibration-input.v1".to_string(),
            output_schema: "grape.calibration-output.v1".to_string(),
