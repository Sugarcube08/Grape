...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let source = path_value(&args, "--capture-sqlite")?
        .ok_or_else(|| grape_core::GrapeError::message("missing required --capture-sqlite path"))?;
    let db = default_path(&args, "--db", "grape.sqlite")?;
    let output = path_value(&args, "--output")?;
    let session_id = value(&args, "--session-id")?
        .ok_or_else(|| grape_core::GrapeError::message("missing required --session-id value"))?;
    let device_model =
        value(&args, "--device-model")?.unwrap_or_else(|| "WHOOP 5.0 Grape".to_string());
    let sensitivity =
