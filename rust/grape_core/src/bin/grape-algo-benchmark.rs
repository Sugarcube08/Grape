...
use grape_core::{
    GrapeError,
    algorithm_compare::{
        AlgorithmComparisonReport, compare_hrv_grape_to_reference,
        compare_sleep_grape_to_external_reference_report, compare_sleep_grape_to_reference,
        compare_sleep_v1_grape_to_reference, compare_strain_grape_to_reference,
        compare_stress_grape_to_reference,
    },
    metrics::{
        AlgorithmRunResult, GRAPE_HRV_V0_ID, GRAPE_SLEEP_V1_ID, HrvInput, RecoveryInput,
        SleepInput, SleepV1Input, StrainInput, StressInput, algorithm_run_record,
        built_in_algorithm_definitions, grape_hrv_v0, grape_recovery_v0, grape_sleep_v0,
        grape_sleep_v1, grape_strain_v0, grape_stress_v0,
    },
    report::write_json_report,
    store::{AlgorithmRunRecord, GrapeStore},
...
fn run() -> grape_core::GrapeResult<()> {
    ...
    if flag(&args, "--compare-reference") {
        ...
        let cli_data_coverage = serde_json::to_value(data_coverage_from_report(
            ...
            &report.start_time,
            &report.end_time,
            report.grape_output.is_some() && report.reference_output.is_some(),
            report.grape_quality_flags.len() + report.reference_quality_flags.len(),
            report.errors.len(),
        ))
        .map_err(|error| {
    ...
    let algorithm_id = value(&args, "--algorithm")?
        ...
        .unwrap_or_else(|| GRAPE_HRV_V0_ID.to_string());
    ...
    if let Some(db_path) = db {
        ...
        let store = GrapeStore::open(&db_path)?;
    ...
    let report = AlgoBenchmarkReport {
        schema: "grape.algo-benchmark-report.v1".to_string(),
        generated_by: "grape-algo-benchmark".to_string(),
        family: run.family,
...
) -> grape_core::GrapeResult<AlgorithmComparisonReport> {
    match family {
        "hrv" => {
        ...
        "sleep" => {
            if algorithm_id == Some(GRAPE_SLEEP_V1_ID) {
...
fn reject_external_reference_report_for_sleep_v1(
    ...
    if reference_report_path.is_some() {
        return Err(GrapeError::message(
            "--reference-report currently compares external sleep reports against grape.sleep.v0; omit it for grape.sleep.v1 reference comparison",
        ));
    }
    Ok(())
}
...
fn run_benchmark_algorithm(
    algorithm_id: &str,
    input_path: &PathBuf,
) -> grape_core::GrapeResult<BenchmarkRun> {
    match algorithm_id {
        "grape.hrv.v0" => run_typed(input_path, |input: HrvInput| grape_hrv_v0(&input)),
        "grape.sleep.v0" => run_typed(input_path, |input: SleepInput| grape_sleep_v0(&input)),
        "grape.sleep.v1" => run_typed(input_path, |input: SleepV1Input| grape_sleep_v1(&input)),
        "grape.strain.v0" => run_typed(input_path, |input: StrainInput| grape_strain_v0(&input)),
        "grape.recovery.v0" => {
            run_typed(input_path, |input: RecoveryInput| grape_recovery_v0(&input))
        }
        "grape.stress.v0" => run_typed(input_path, |input: StressInput| grape_stress_v0(&input)),
        other => Err(GrapeError::message(format!(
            "unsupported algorithm {other}; use metrics.built_in_definitions or --algorithm grape.hrv.v0|grape.sleep.v0|grape.sleep.v1|grape.strain.v0|grape.recovery.v0|grape.stress.v0"
        ))),
    }
}
    ...
    O: Serialize,
{
    ...
    let input: I = serde_json::from_value(input_value.clone())
        .map_err(|error| GrapeError::message(format!("cannot parse benchmark input: {error}")))?;
    ...
    let output = result
        ...
        .transpose()
        .map_err(|error| GrapeError::message(format!("cannot serialize output: {error}")))?;
...
fn benchmark_label_args(args: &[String]) -> grape_core::GrapeResult<Option<BenchmarkLabelArgs>> {
    let Some(value_raw) = value(args, "--label-value")? else {
        return Ok(None);
    };
    let label_value = value_raw
        .parse::<f64>()
        .map_err(|error| GrapeError::message(format!("invalid --label-value: {error}")))?;
    ...
    let provenance = serde_json::from_str(&provenance_raw)
        .map_err(|error| GrapeError::message(format!("invalid label provenance JSON: {error}")))?;
    let max_absolute_error = value(args, "--max-absolute-error")?
        .map(|raw| {
            raw.parse::<f64>().map_err(|error| {
                GrapeError::message(format!("invalid --max-absolute-error: {error}"))
            })
        })
        .transpose()?;
...
fn read_typed_input<I>(input_path: &PathBuf) -> grape_core::GrapeResult<I>
where
    I: DeserializeOwned,
{
    let input_raw =
...
fn read_json_value(path: &PathBuf) -> grape_core::GrapeResult<serde_json::Value> {
    let raw = fs::read_to_string(path).map_err(|source| GrapeError::io(path, source))?;
    serde_json::from_str(&raw).map_err(|source| GrapeError::json(path, source))
}

fn input_path(args: &[String], algorithm_id: &str) -> grape_core::GrapeResult<PathBuf> {
    if let Some(path) = path_value(args, "--input")? {
        return Ok(path);
    }
    if algorithm_id == GRAPE_HRV_V0_ID {
        return Ok(PathBuf::from(
            "fixtures/synthetic/hrv_grape_v0_hand_derived.json",
        ));
    }
...
fn input_path_for_family(args: &[String], family: &str) -> grape_core::GrapeResult<PathBuf> {
    if let Some(path) = path_value(args, "--input")? {
        return Ok(path);
    }
    match family {
        ...
        "sleep" => Ok(PathBuf::from(
            "fixtures/synthetic/sleep_grape_v0_hand_derived.json",
        )),
        "strain" => Ok(PathBuf::from(
            "fixtures/synthetic/strain_grape_v0_hand_derived.json",
        )),
        other => Err(GrapeError::message(format!(
...
fn default_algorithm_for_family(family: &str) -> Option<String> {
    match family {
        "hrv" => Some("grape.hrv.v0".to_string()),
        "sleep" => Some("grape.sleep.v0".to_string()),
        "strain" => Some("grape.strain.v0".to_string()),
        "recovery" => Some("grape.recovery.v0".to_string()),
        "stress" => Some("grape.stress.v0".to_string()),
        _ => None,
    }
}
