...
const MANIFEST_SCHEMA: &str = "grape.local-health-validation-manifest.v1";
const REPORT_SCHEMA: &str = "grape.local-health-validation-suite-report.v1";
...
fn run() -> grape_core::GrapeResult<()> {
    ...
    let manifest_path = path_value(&args, "--manifest")?
        .ok_or_else(|| GrapeError::message("--manifest is required"))?;
    let output = path_value(&args, "--output")?;
    let markdown_output = path_value(&args, "--markdown-output")?;
    let review_output = path_value(&args, "--review-output")?;
...
fn raw_export_bundle_path_value(args: &[String]) -> grape_core::GrapeResult<Option<PathBuf>> {
...
fn resolve_validation_database(
...
) -> grape_core::GrapeResult<ResolvedValidationDatabase> {
    match (database_path, raw_export_bundle_path) {
        ...
        (Some(_), Some(_)) => Err(GrapeError::message(
            "use either --database or --raw-export-bundle, not both",
        )),
        (None, None) => Err(GrapeError::message(
...
fn resolve_raw_export_bundle_database(
    ...
    if bundle_path.is_dir() {
        let database_path = bundle_path.join("data").join("grape.sqlite");
        ...
        return Err(GrapeError::message(format!(
            "raw export bundle {} is missing data/grape.sqlite",
            bundle_path.display()
        )));
    }
...
fn audit_validation_sqlite(
    ...
    let storage_schema_version = match connection.query_row(
        "SELECT MAX(version) FROM grape_schema_migrations",
        [],
...
fn sqlite_table_exists(connection: &Connection, table_name: &str) -> grape_core::GrapeResult<bool> {
    connection
        .query_row(
        ...
        .map(|value| value != 0)
        .map_err(|error| GrapeError::message(format!("cannot inspect SQLite tables: {error}")))
}
...
fn audit_raw_export_zip_manifest(
    ...
    let manifest_raw = (|| -> grape_core::GrapeResult<String> {
        let file = File::open(bundle_path).map_err(|source| GrapeError::io(bundle_path, source))?;
...
fn raw_export_manifest_audit_from_raw(
    ...
    let mut issues = Vec::new();
    if manifest.schema_version != "grape.export.v1" {
        issues.push(format!(
    ...
    let sqlite_files = manifest
        .files
        .iter()
        .filter(|file| file.path == "data/grape.sqlite")
        .collect::<Vec<_>>();
...
fn raw_export_zip_entry_prefix(sqlite_archive_entry: &str) -> String {
    sqlite_archive_entry
        .strip_suffix("data/grape.sqlite")
        .unwrap_or("")
        .to_string()
}

fn file_sha256_hex(path: &Path) -> grape_core::GrapeResult<String> {
    let bytes = fs::read(path).map_err(|source| GrapeError::io(path, source))?;
    Ok(sha256_hex(&bytes))
}
...
fn extract_raw_export_sqlite_from_zip(
    bundle_path: &Path,
) -> grape_core::GrapeResult<ExtractedRawExportSqlite> {
    let file = File::open(bundle_path).map_err(|source| GrapeError::io(bundle_path, source))?;
    let mut archive = ZipArchive::new(file).map_err(|error| {
        GrapeError::message(format!(
            "raw export bundle {} is not a readable zip: {error}",
    ...
    let entry_name = raw_export_sqlite_zip_entry_name(&archive).ok_or_else(|| {
        GrapeError::message(format!(
            "raw export zip {} is missing data/grape.sqlite",
            bundle_path.display()
        ))
    })?;
    ...
    let mut output =
        File::create(&extracted_path).map_err(|source| GrapeError::io(&extracted_path, source))?;
    io::copy(&mut entry, &mut output).map_err(|source| GrapeError::io(&extracted_path, source))?;
...
fn raw_export_sqlite_zip_entry_name<R: io::Read + io::Seek>(
    ...
    let matches = archive
        ...
        .filter_map(|name| {
            ...
            if normalized == "data/grape.sqlite" || normalized.ends_with("/data/grape.sqlite") {
                Some(name.to_string())
            } else {
                None
            }
...
fn read_manifest_value(path: &Path) -> grape_core::GrapeResult<Value> {
    let raw = fs::read_to_string(path).map_err(|source| GrapeError::io(path, source))?;
    serde_json::from_str(&raw)
        .map_err(|source| GrapeError::message(format!("invalid validation manifest: {source}")))
}

fn parse_manifest(value: Value) -> grape_core::GrapeResult<LocalHealthValidationManifest> {
...
fn run_manifest(
    ...
    LocalHealthValidationSuiteReport {
        ...
        generated_by: "grape-local-health-validation-suite".to_string(),
...
fn write_markdown_report(
    ...
    path: &Path,
) -> grape_core::GrapeResult<()> {
    write_markdown_text(&markdown_report(report), path)
}

fn write_json_file<T: Serialize>(report: &T, path: &Path) -> grape_core::GrapeResult<()> {
...
fn write_markdown_text(markdown: &str, path: &Path) -> grape_core::GrapeResult<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|source| GrapeError::io(parent, source))?;
    }
    fs::write(path, markdown.as_bytes()).map_err(|source| GrapeError::io(path, source))
}
...
fn run_capture_sqlite_import(
    ...
    let device_model = import
        .device_model
        .clone()
        .unwrap_or_else(|| "WHOOP 5.0 Grape".to_string());
...
fn metric_records_for_case(
    ...
    match str_field(result, &["schema"]).as_deref() {
        Some("grape.step-packet-discovery-report.v1") => {
            vec![step_discovery_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("grape.step-capture-validation-report.v1") => {
            vec![step_validation_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("grape.raw-motion-step-estimate-report.v1") => {
            vec![raw_motion_step_metric_record(
                case_id, report, method, result,
            )]
        }
        Some("grape.step-counter-daily-rollup-report.v1") => {
        ...
        Some("grape.step-counter-hourly-rollup-report.v1") => {
            vec![step_counter_rollup_metric_record(
                case_id,
                report,
                method,
...
fn step_discovery_metric_record(
    ...
    metric_record(
        ...
        None,
        Some("grape.steps.step_packet_discovery.v0".to_string()),
...
fn step_validation_metric_record(
    ...
    metric_record(
        ...
        None,
        Some("grape.steps.device_counter.v0".to_string()),
        Some("0.1.0".to_string()),
...
fn energy_rollup_metric_records(
    ...
    .map(|(metric_name, unit)| {
        ...
        metric_record(
            ...
            f64_field(result, &["confidence"]),
            Some("grape.energy.local_estimate.v0".to_string()),
            Some("0.1.0".to_string()),
            None,
            Some(promotion_status.to_string()),
            string_array(result, &["issues"]),
    ...
    result
        ...
        .map(|status| {
            ...
            metric_record(
                ...
                Some(0.0),
                Some("grape.energy.unavailable_status.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
...
fn rhr_rollup_metric_record(
    ...
    metric_record(
        ...
        f64_field(result, &["confidence"]),
        Some("grape.resting_heart_rate.device_sensor.v0".to_string()),
        Some("0.1.0".to_string()),
        None,
    ...
    result
        ...
        .map(|status| {
            ...
            metric_record(
                ...
                Some("grape.recovery_sensor.device_sensor.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
                str_field(status, &["promotion_status"]),
                Vec::new(),
    ...
    result
        ...
        .map(|status| {
            ...
            metric_record(
                ...
                Some(0.0),
                Some("grape.recovery.unavailable_status.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
                str_field(status, &["promotion_status"]),
                Vec::new(),
    ...
    result
        ...
        .map(|status| {
            metric_record(
                ...
                Some(0.0),
                Some("grape.activity.unavailable_status.v0".to_string()),
                Some("0.1.0".to_string()),
                None,
                str_field(status, &["promotion_status"]),
                Vec::new(),
...
fn scoped_database_path_for_case(
...
) -> grape_core::GrapeResult<Option<PathBuf>> {
    ...
    {
        let _ = GrapeStore::open(&scoped_path)?;
    }
...
fn copy_capture_session_scope(
    ...
    let connection = Connection::open(scoped_path).map_err(|error| {
        GrapeError::message(format!("cannot open scoped validation database: {error}"))
    })?;
    ...
    connection
        ...
        .map_err(|error| GrapeError::message(format!("cannot attach source database: {error}")))?;

    let copy_result = (|| -> grape_core::GrapeResult<()> {
        connection
            .execute_batch(&format!(
                r#"
            ...
            .map_err(|error| {
                GrapeError::message(format!("cannot copy scoped packet evidence: {error}"))
            })?;
                ...
                parse_rfc3339_utc_unix_ms(&case.start).zip(parse_rfc3339_utc_unix_ms(&case.end))
        {
            connection
                ...
                .map_err(|error| {
                    GrapeError::message(format!("cannot copy scoped step-counter samples: {error}"))
                })?;
        }
        Ok(())
    })();
    ...
    let detach_result = connection
        ...
        .map_err(|error| GrapeError::message(format!("cannot detach source database: {error}")));
    copy_result.and(detach_result)
}

fn source_table_exists(connection: &Connection, table_name: &str) -> grape_core::GrapeResult<bool> {
    connection
        .query_row(
        ...
        .map(|value| value != 0)
        .map_err(|error| GrapeError::message(format!("cannot inspect source tables: {error}")))
}

fn persist_scoped_formatted_metric_writes(
    source_database_path: &str,
    scoped_path: &Path,
) -> grape_core::GrapeResult<usize> {
    ...
    connection
        ...
        .map_err(|error| GrapeError::message(format!("cannot attach scoped database: {error}")))?;

    let copy_result = (|| -> grape_core::GrapeResult<usize> {
        ...
        for table in [
            ...
            changed += connection
                ...
                .map_err(|error| {
                    GrapeError::message(format!("cannot copy scoped {table}: {error}"))
                })?;
        }
        Ok(changed)
    })();
    ...
    let detach_result = connection
        ...
        .map_err(|error| GrapeError::message(format!("cannot detach scoped database: {error}")));
...
fn unique_validation_temp_path(token: &str) -> PathBuf {
    ...
    env::temp_dir().join(format!(
        "grape-local-health-validation-{}-{}-{}.sqlite",
        process::id(),
...
fn suite_issue_action(issue: &str) -> &'static str {
    match issue {
        ...
        issue if issue.starts_with("manifest_json_invalid") => {
            "Regenerate manifest.json from grape-raw-export so it matches the Grape Raw Export manifest schema."
        }
        issue if issue.starts_with("manifest_schema_version_unexpected") => {
            "Regenerate the Raw Export bundle with schema_version grape.export.v1."
        }
        ...
        issue if issue.starts_with("case_window_packet_evidence_query_failed") => {
            "Repair or regenerate data/grape.sqlite so raw and decoded packet evidence can be inspected for each validation case window."
        }
        issue
        ...
        {
            "Repair or regenerate data/grape.sqlite so decoded packet families can be inspected for each validation case window."
        }
        issue
        ...
        {
            "Repair or regenerate data/grape.sqlite so packet timestamp bounds can be inspected for each validation case window."
        }
        "sqlite_family_not_declared" => {
            "Regenerate the Raw Export bundle with the sqlite family selected so data/grape.sqlite is declared."
        }
        "sqlite_manifest_file_missing" => {
            "Regenerate the Raw Export bundle so manifest.json includes data/grape.sqlite."
        }
        "sqlite_manifest_file_duplicate" => {
            "Regenerate the Raw Export bundle so manifest.json lists data/grape.sqlite exactly once."
        }
        issue if issue.starts_with("sqlite_manifest_kind_unexpected") => {
            "Regenerate the Raw Export bundle so data/grape.sqlite is declared with kind sqlite."
        }
        "sqlite_manifest_sha256_missing" => {
            "Regenerate the Raw Export bundle so data/grape.sqlite has a manifest SHA-256."
        }
        "sqlite_actual_sha256_unavailable" => {
            "Repair data/grape.sqlite permissions or regenerate the Raw Export bundle before validation."
        }
        "sqlite_manifest_sha256_mismatch" => {
            "Regenerate the Raw Export bundle; manifest.json and data/grape.sqlite do not describe the same SQLite snapshot."
        }
        ...
        issue if issue.starts_with("sqlite_open_failed") => {
            "Regenerate the Raw Export bundle; data/grape.sqlite is not an openable SQLite database."
        }
        issue
            if issue.starts_with("sqlite_schema_version_query_failed")
                || issue == "sqlite_schema_version_missing" =>
        {
            "Regenerate the Raw Export bundle from migrated Grape storage so grape_schema_migrations is present."
        }
        issue if issue.starts_with("sqlite_required_table_missing") => {
            "Regenerate the Raw Export bundle from Grape storage that includes local health evidence and formatted metric tables."
        }
        issue
        ...
        {
            "Repair or regenerate data/grape.sqlite so canonical Grape tables can be inspected."
        }
        ...
        "capture_sqlite_raw_import_incomplete" => {
            "Repair the target Grape DB or raw_evidence constraints, then rerun the capture.sqlite import."
        }
