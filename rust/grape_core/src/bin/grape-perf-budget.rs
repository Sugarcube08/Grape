    ...
    Ok(value(args, name)?
        .map(|raw| {
            raw.parse::<usize>().map_err(|source| {
                grape_core::GrapeError::message(format!("invalid {name}: {source}"))
            })
        })
        .transpose()?)
}

fn optional_u64(args: &[String], name: &str) -> grape_core::GrapeResult<Option<u64>> {
    Ok(value(args, name)?
        .map(|raw| {
...
fn optional_mib(args: &[String], name: &str) -> grape_core::GrapeResult<Option<u64>> {
