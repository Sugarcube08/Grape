...
use crate::{
    ...
    store::{DailyActivityMetricInput, DecodedFrameRow, GrapeStore, MetricProvenanceInput},
    validation_labels::{
...
pub const RAW_MOTION_STEP_ESTIMATE_REPORT_SCHEMA: &str = "grape.raw-motion-step-estimate-report.v1";
pub const GRAPE_STEPS_RAW_MOTION_ESTIMATE_V0_ID: &str = "grape.steps.raw_motion_estimate.v0";
pub const GRAPE_STEPS_RAW_MOTION_ESTIMATE_V0_VERSION: &str = "0.1.0";
...
) -> GrapeResult<RawMotionStepEstimateReport> {
...
pub fn run_raw_motion_step_estimate(
    ...
    Ok(RawMotionStepEstimateReport {
        ...
        generated_by: "grape-raw-motion-step-estimator".to_string(),
        pass,
        database_path: database_path.to_string(),
        start: start.to_string(),
        end: end.to_string(),
        algorithm_id: GRAPE_STEPS_RAW_MOTION_ESTIMATE_V0_ID.to_string(),
        algorithm_version: GRAPE_STEPS_RAW_MOTION_ESTIMATE_V0_VERSION.to_string(),
...
fn persist_validated_raw_motion_step_metric(
    ...
    let date_key = options
        ...
        .ok_or_else(|| GrapeError::message("date_key is required when write_metric is true"))?;
    let timezone = options
        .timezone
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| GrapeError::message("timezone is required when write_metric is true"))?;
    let start_time_unix_ms = report.start_time_unix_ms.ok_or_else(|| {
        GrapeError::message("start must be an RFC3339 UTC timestamp when write_metric is true")
    })?;
    let end_time_unix_ms = report.end_time_unix_ms.ok_or_else(|| {
        GrapeError::message("end must be an RFC3339 UTC timestamp when write_metric is true")
    })?;
    ...
    let estimated_steps = report.estimated_steps.ok_or_else(|| {
        GrapeError::message("estimated_steps is required before writing a step metric")
    })?;
    ...
    let provenance_json = json!({
        "algorithm": GRAPE_STEPS_RAW_MOTION_ESTIMATE_V0_ID,
        "algorithm_version": GRAPE_STEPS_RAW_MOTION_ESTIMATE_V0_VERSION,
...
fn estimate_frame_steps(
    ...
    options: &RawMotionStepEstimateOptions,
) -> GrapeResult<Option<RawMotionStepFrameEstimate>> {
...
fn validate_options(options: &RawMotionStepEstimateOptions) -> GrapeResult<()> {
    ...
    if options.tolerance_steps < 0 {
        return Err(GrapeError::message("tolerance_steps must be non-negative"));
    }
