...
pub const FIXTURE_INDEX_SCHEMA: &str = "grape.fixture-index.v1";
pub const FRAME_HEX_SCHEMA: &str = "grape.frame.hex.v1";
pub const CAPTURED_FRAME_BATCH_SCHEMA: &str = "grape.captured-frame-batch.v1";
pub const PAYLOAD_HEX_SCHEMA: &str = "grape.payload.hex.v1";
pub const ACTIVITY_SESSION_FIXTURE_SCHEMA: &str = "grape.activity-session-fixtures.v1";
pub const OPENWHOOP_REFERENCE_FIXTURE_SCHEMA: &str = "grape.openwhoop-reference-fixture.v1";
pub const COMMAND_VALIDATION_FIXTURE_SCHEMA: &str = "grape.command-validation-fixtures.v1";
...
pub fn build_fixture_index(root: &Path) -> GrapeResult<FixtureIndexReport> {
    ...
    for sidecar in sidecars {
        let raw =
            fs::read_to_string(&sidecar).map_err(|source| GrapeError::io(&sidecar, source))?;
        let metadata: FixtureMetadata =
            serde_json::from_str(&raw).map_err(|source| GrapeError::json(&sidecar, source))?;
    ...
    Ok(FixtureIndexReport {
        ...
        generated_by: "grape-fixture-index".to_string(),
...
pub fn run_parser_fixtures(root: &Path, index: &FixtureIndexReport) -> ParserFixtureReport {
    ...
    if results.is_empty() {
        issues.push("no grape.frame.hex.v1 fixtures found".to_string());
    }

    let next_actions = parser_fixture_report_next_actions(&issues, &results);

    ParserFixtureReport {
        schema: "grape.parser-fixture-report.v1".to_string(),
        generated_by: "grape-parser-fixture-runner".to_string(),
        fixture_root: root.display().to_string(),
        pass: issues.is_empty(),
        fixtures: results,
        issues,
...
pub fn load_fixture_index(path: &Path) -> GrapeResult<FixtureIndexReport> {
...
fn run_frame_fixture(root: &Path, fixture: &IndexedFixture) -> ParserFixtureResult {
    ...
    let device_type = expected_device_type(&expected).unwrap_or(DeviceType::Grape);
...
fn parser_fixture_report_next_actions(
    ...
    for issue in issues {
        if issue == "no grape.frame.hex.v1 fixtures found" {
            actions.push(FixtureNextAction {
                scope: "parser_fixtures".to_string(),
                reason: "no_parser_fixtures".to_string(),
                action: "Add at least one grape.frame.hex.v1, grape.payload.hex.v1, or grape.captured-frame-batch.v1 fixture and regenerate the fixture index.".to_string(),
            });
...
fn parser_fixture_issue_action(issue: &str) -> (&'static str, &'static str) {
    if issue.starts_with("cannot read fixture file:") {
        ...
        (
            ...
            "Update the captured-frame batch schema to grape.captured-frame-batch.v1 or add a migration before trusting it.",
        )
...
fn collect_sidecars(root: &Path, sidecars: &mut Vec<PathBuf>) -> GrapeResult<()> {
    if !root.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(root).map_err(|source| GrapeError::io(root, source))? {
        let entry = entry.map_err(|source| GrapeError::io(root, source))?;
        let path = entry.path();
        if path.is_dir() {
...
fn collect_data_files(root: &Path, files: &mut Vec<PathBuf>) -> GrapeResult<()> {
    if !root.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(root).map_err(|source| GrapeError::io(root, source))? {
