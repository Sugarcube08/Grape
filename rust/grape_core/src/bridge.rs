...
use crate::{
    GrapeError, GrapeResult,
    activity_sessions::{
        ActivitySessionCorrectionKind, activity_session_correction_plans,
        append_activity_session_correction_history,
    },
    algorithm_compare::{
        compare_hrv_grape_to_reference, compare_sleep_grape_to_external_reference_report,
        compare_sleep_grape_to_reference, compare_sleep_v1_grape_to_external_reference_report,
        compare_sleep_v1_grape_to_reference, compare_strain_grape_to_reference,
        compare_stress_grape_to_reference,
    },
    calibration::{
    ...
    metrics::{
        AlgorithmRunResult, GRAPE_HRV_V0_ID, GRAPE_HRV_V0_VERSION, GRAPE_RECOVERY_V0_ID,
        GRAPE_RECOVERY_V0_VERSION, GRAPE_SLEEP_V0_ID, GRAPE_SLEEP_V0_VERSION, GRAPE_SLEEP_V1_ID,
        GRAPE_SLEEP_V1_VERSION, GRAPE_STRAIN_V0_ID, GRAPE_STRAIN_V0_VERSION, GRAPE_STRESS_V0_ID,
        GRAPE_STRESS_V0_VERSION, HrvInput, RecoveryInput, SleepInput, SleepModelStatusInput,
        ...
        grape_hrv_v0, grape_recovery_v0, grape_sleep_v0, grape_sleep_v1, grape_strain_v0,
        grape_stress_v0, sleep_history_night_is_usable,
    },
    openwhoop_reference::{
    ...
    store::{
        ...
        ExternalSleepSessionRow, ExternalSleepStageInput, ExternalSleepStageRow, GrapeStore,
...
pub const BRIDGE_REQUEST_SCHEMA: &str = "grape.bridge.request.v1";
pub const BRIDGE_RESPONSE_SCHEMA: &str = "grape.bridge.response.v1";
pub const CAPTURE_ARRIVAL_PLAN_REPORT_SCHEMA: &str = "grape.capture-arrival-plan-report.v1";
...
struct ReferenceCompareArgs {
    ...
    #[serde(default)]
    grape_algorithm_id: Option<String>,
}
...
pub fn core_version_payload() -> serde_json::Value {
    json!({
        ...
        "crate_name": option_env!("CARGO_PKG_NAME").unwrap_or("grape-core"),
...
pub fn openwhoop_reference_report_payload() -> serde_json::Value {
    ...
    let history_fields = openwhoop_history_field_references()
        ...
        .map(|reference| {
            json!({
                ...
                "gen4": reference.gen4,
                "gen5": reference.gen5,
                "grape_summary_kinds": reference.grape_summary_kinds,
    ...
    json!({
        "schema": "grape.openwhoop-reference-report.v1",
        "generated_by": "grape-bridge",
        "snapshot": {
...
fn handle_bridge_request_inner(request: BridgeRequest) -> BridgeResponse {
    ...
    match request.method.as_str() {
        ...
        "metrics.built_in_definitions" => serde_json::to_value(built_in_algorithm_definitions())
            .map_err(|error| GrapeError::message(error.to_string()))
        ...
        "metrics.grape_hrv_v0" => request_args::<HrvInput>(&request)
            .and_then(|input| metric_result_to_value(grape_hrv_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.grape_sleep_v0" => request_args::<SleepInput>(&request)
            .and_then(|input| metric_result_to_value(grape_sleep_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.grape_sleep_v1" => request_args::<SleepV1Input>(&request)
            .and_then(|input| metric_result_to_value(grape_sleep_v1(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.grape_strain_v0" => request_args::<StrainInput>(&request)
            .and_then(|input| metric_result_to_value(grape_strain_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.grape_recovery_v0" => request_args::<RecoveryInput>(&request)
            .and_then(|input| metric_result_to_value(grape_recovery_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
        "metrics.grape_stress_v0" => request_args::<StressInput>(&request)
            .and_then(|input| metric_result_to_value(grape_stress_v0(&input)))
            .map(|value| bridge_ok(&request.request_id, value))
            .unwrap_or_else(|error| bridge_error(&request.request_id, "method_error", error)),
...
#[unsafe(no_mangle)]
pub extern "C" fn grape_core_version_json() -> *mut c_char {
    json_to_c_string(core_version_payload())
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn grape_bridge_handle_json(request_json: *const c_char) -> *mut c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn grape_bridge_free_string(value: *mut c_char) {
    if value.is_null() {
        return;
    }
...
fn parse_frame_hex_bridge(args: ParseFrameArgs) -> GrapeResult<serde_json::Value> {
    let device_type = parse_device_type(&args.device_type)?;
    let parsed = parse_frame_hex(device_type, &args.frame_hex)?;
    serde_json::to_value(parsed)
        .map_err(|error| GrapeError::message(format!("cannot serialize parsed frame: {error}")))
}

fn parse_frame_hex_batch_bridge(args: ParseFrameBatchArgs) -> GrapeResult<serde_json::Value> {
    let device_type = parse_device_type(&args.device_type)?;
...
fn timeline_from_decoded_frames_bridge(args: TimelineArgs) -> GrapeResult<serde_json::Value> {
    let rows = packet_timeline_from_decoded_frames(&args.decoded_frames)?;
    serde_json::to_value(rows)
        .map_err(|error| GrapeError::message(format!("cannot serialize timeline rows: {error}")))
}

fn storage_check_bridge(args: StorageCheckArgs) -> GrapeResult<serde_json::Value> {
    if args.database_path.trim().is_empty() {
        return Err(GrapeError::message("database_path is required"));
    }
    ...
    serde_json::to_value(report)
        .map_err(|error| GrapeError::message(format!("cannot serialize storage report: {error}")))
}

fn apply_default_preferences_bridge(
    args: ApplyDefaultPreferencesArgs,
) -> GrapeResult<serde_json::Value> {
    ...
    serde_json::to_value(preferences)
        .map_err(|error| GrapeError::message(format!("cannot serialize preferences: {error}")))
}

fn set_algorithm_preference_bridge(args: SetPreferenceArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(preference)
        .map_err(|error| GrapeError::message(format!("cannot serialize preference: {error}")))
}

fn get_algorithm_preference_bridge(args: GetPreferenceArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn list_algorithm_preferences_bridge(args: ListPreferencesArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn evaluate_calibration_dataset_bridge(
    ...
    let mut value = serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize calibration report: {error}"))
    })?;
...
fn evaluate_stored_calibration_labels_bridge(
    ...
    if args.start.trim().is_empty() {
        return Err(GrapeError::message("start is required"));
    }
    if args.end.trim().is_empty() {
        return Err(GrapeError::message("end is required"));
    }
    if args.start >= args.end {
        return Err(GrapeError::message("start must be earlier than end"));
    }
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert(
            ...
            json!("grape.calibration-dataset.v1"),
        );
        object.insert(
...
fn import_calibration_labels_bridge(
    ...
    for label in args.labels {
        let provenance_json = serde_json::to_string(&label.provenance).map_err(|error| {
            GrapeError::message(format!("cannot serialize label provenance: {error}"))
        })?;
    ...
    Ok(json!({
        "schema": "grape.calibration-label-import-report.v1",
        "generated_by": "grape-bridge",
        "pass": true,
...
fn list_calibration_labels_bridge(
    ...
    let labels = store.calibration_labels_between(&args.start, &args.end)?;
    Ok(json!({
        "schema": "grape.calibration-label-list.v1",
        "generated_by": "grape-bridge",
        "start": args.start,
        "end": args.end,
...
fn apply_calibration_bridge(args: ApplyCalibrationArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let calibration_run = match args.calibration_run_id.as_deref() {
        Some(calibration_run_id) if !calibration_run_id.trim().is_empty() => {
            store.calibration_run(calibration_run_id)?.ok_or_else(|| {
                GrapeError::message(format!("calibration run {calibration_run_id} not found"))
            })?
        }
    ...
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize calibration application: {error}"))
    })
}
...
fn maybe_persist_calibration_report(
    ...
    let database_path = database_path
        .ok_or_else(|| GrapeError::message("database_path is required when persist is true"))?;
    let calibration_run_id = calibration_run_id
        .filter(|value| !value.trim().is_empty())
        .ok_or_else(|| {
            GrapeError::message("calibration_run_id is required when persist is true")
        })?;
...
fn stored_calibration_dataset(
    ...
    (
        CalibrationDataset {
            schema: "grape.calibration-dataset.v1".to_string(),
            records,
        },
        matched_records,
        issues,
...
fn raw_export_bridge(args: RawExportArgs) -> GrapeResult<serde_json::Value> {
    if args.output_dir.trim().is_empty() {
        return Err(GrapeError::message("output_dir is required"));
    }
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(report)
        .map_err(|error| GrapeError::message(format!("cannot serialize export report: {error}")))
}

fn export_validate_bundle_bridge(args: ExportValidateBundleArgs) -> GrapeResult<serde_json::Value> {
    if args.path.trim().is_empty() {
        return Err(GrapeError::message("path is required"));
    }
...
fn local_health_validation_manifest_runbook_bridge(
    ...
    if !args.manifest.is_object() {
        return Err(GrapeError::message("manifest object is required"));
    }
    ...
    let manifest_schema = args
        ...
        .unwrap_or("unknown");
    Ok(serde_json::json!({
        "schema": "grape.local-health-validation-runbook.v1",
...
fn privacy_lint_bridge(args: PrivacyLintArgs) -> GrapeResult<serde_json::Value> {
    if args.path.trim().is_empty() {
        return Err(GrapeError::message("path is required"));
    }
    let report = lint_privacy_path(Path::new(&args.path))?;
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize privacy lint report: {error}"))
    })
}
...
fn capture_sanitize_bridge(args: CaptureSanitizeArgs) -> GrapeResult<serde_json::Value> {
    if args.input_path.trim().is_empty() {
        return Err(GrapeError::message("input_path is required"));
    }
    if args.output_path.trim().is_empty() {
        return Err(GrapeError::message("output_path is required"));
    }
    ...
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize capture sanitize report: {error}"))
    })
}

fn ui_coverage_audit_bridge(args: UiCoverageAuditArgs) -> GrapeResult<serde_json::Value> {
    let input_path = args
        .coverage_map_path
    ...
    let input_raw =
        fs::read_to_string(&input_path).map_err(|source| GrapeError::io(&input_path, source))?;
    let input: UiCoverageAuditInput =
        serde_json::from_str(&input_raw).map_err(|source| GrapeError::json(&input_path, source))?;
...
fn perf_budget_bridge(args: PerfBudgetArgs) -> GrapeResult<serde_json::Value> {
    let report = run_perf_budget(PerfBudgetOptions {
        scale: args.scale,
        budgets: PerfBudgets::default(),
    })?;
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize perf budget report: {error}"))
    })
}

fn property_suite_bridge(args: PropertySuiteArgs) -> GrapeResult<serde_json::Value> {
    let report = run_property_suite(PropertySuiteOptions {
        seed: args.seed,
        cases_per_group: args.cases_per_group,
    })?;
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize property suite report: {error}"))
    })
}

fn reference_compare_bridge(args: ReferenceCompareArgs) -> GrapeResult<serde_json::Value> {
    let report = match args.family.as_str() {
        "hrv" => {
            let input: HrvInput = serde_json::from_value(args.input)
                .map_err(|error| GrapeError::message(format!("invalid HRV input: {error}")))?;
            compare_hrv_grape_to_reference(&input)?
        }
        "sleep" => {
            let use_sleep_v1 = args
                .grape_algorithm_id
                .as_deref()
                .is_some_and(|id| id == crate::metrics::GRAPE_SLEEP_V1_ID)
                || args
                    .input
                    .get("sleep")
            ...
            if use_sleep_v1 {
                ...
                .map_err(|error| GrapeError::message(format!("invalid sleep v1 input: {error}")))?;
                if let Some(reference_report) = args.reference_report {
                    compare_sleep_v1_grape_to_external_reference_report(&input, &reference_report)?
                } else {
                    compare_sleep_v1_grape_to_reference(&input)?
                }
            } else {
                let input: SleepInput = serde_json::from_value(args.input).map_err(|error| {
                    GrapeError::message(format!("invalid sleep input: {error}"))
                })?;
                if let Some(reference_report) = args.reference_report {
                    compare_sleep_grape_to_external_reference_report(&input, &reference_report)?
                } else {
                    compare_sleep_grape_to_reference(&input)?
                }
            }
        }
        "strain" => {
            let input: StrainInput = serde_json::from_value(args.input)
                .map_err(|error| GrapeError::message(format!("invalid strain input: {error}")))?;
            compare_strain_grape_to_reference(&input)?
        }
        "stress" => {
            let input: StressInput = serde_json::from_value(args.input)
                .map_err(|error| GrapeError::message(format!("invalid stress input: {error}")))?;
            compare_stress_grape_to_reference(&input)?
        }
        other => {
            return Err(GrapeError::message(format!(
...
fn metric_input_readiness_bridge(args: MetricInputReadinessArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn motion_features_bridge(args: MotionFeaturesArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize motion feature report: {error}"))
    })
}

fn heart_rate_features_bridge(args: HeartRateFeaturesArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn vital_event_features_bridge(args: VitalEventFeaturesArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn step_packet_discovery_bridge(args: StepPacketDiscoveryArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn step_counter_ingest_bridge(args: StepCounterIngestArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn daily_activity_metrics_bridge(
    ...
    Ok(json!({
        "schema": "grape.daily-activity-metric-list.v1",
        "generated_by": "grape-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
...
fn hourly_activity_metrics_bridge(
    ...
    Ok(json!({
        "schema": "grape.hourly-activity-metric-list.v1",
        "generated_by": "grape-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
        "metric_count": metrics.len(),
...
fn daily_recovery_metrics_bridge(
    ...
    Ok(json!({
        "schema": "grape.daily-recovery-metric-list.v1",
        "generated_by": "grape-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
        "metric_count": metrics.len(),
...
fn energy_daily_rollup_bridge(args: EnergyDailyRollupArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn energy_hourly_rollup_bridge(args: EnergyHourlyRollupArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn hrv_features_bridge(args: HrvFeaturesArgs) -> GrapeResult<serde_json::Value> {
    ...
    let mut value = serde_json::to_value(&report).map_err(|error| {
        GrapeError::message(format!("cannot serialize HRV feature report: {error}"))
    })?;
...
fn hrv_capture_validation_bridge(args: HrvCaptureValidationArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn metric_window_features_bridge(args: MetricWindowFeaturesArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn sleep_v1_input_from_feature_score(
    ...
    history_import_in_progress: bool,
) -> GrapeResult<SleepV1Input> {
    let prior_history_end_unix_ms = sleep_time_unix_ms(&sleep_input.start_time)
        .ok_or_else(|| GrapeError::message("sleep_v1_input_start_time_invalid"))?;
    ...
    let trusted_grape_sleep_nights = u32::from(
        report
            .sleep_window
            .as_ref()
...
fn external_sleep_history_nights_for_sleep_v1(
    ...
    before_unix_ms: i64,
) -> GrapeResult<Vec<SleepNightHistoryInput>> {
...
fn sleep_feature_score_bridge(args: SleepFeatureScoreArgs) -> GrapeResult<serde_json::Value> {
    let requested_algorithm_id = args
        .algorithm_id
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(GRAPE_SLEEP_V0_ID);
    let requested_algorithm_version = args
        .algorithm_version
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(if requested_algorithm_id == GRAPE_SLEEP_V1_ID {
            GRAPE_SLEEP_V1_VERSION
        } else {
            GRAPE_SLEEP_V0_VERSION
        });
    let sleep_v1_requested = match (requested_algorithm_id, requested_algorithm_version) {
        (GRAPE_SLEEP_V0_ID, GRAPE_SLEEP_V0_VERSION) => false,
        (GRAPE_SLEEP_V1_ID, GRAPE_SLEEP_V1_VERSION) => true,
        _ => {
            return Err(GrapeError::message(format!(
                "unsupported primary algorithm {requested_algorithm_id}@{requested_algorithm_version} for sleep; this packet-derived scorer currently supports {GRAPE_SLEEP_V0_ID}@{GRAPE_SLEEP_V0_VERSION} and {GRAPE_SLEEP_V1_ID}@{GRAPE_SLEEP_V1_VERSION}"
            )));
        }
    };
    let store = open_bridge_store(&args.database_path)?;
    ...
    if sleep_v1_requested {
        if let Some(sleep_input) = report.sleep_input.as_ref() {
            ...
            let sleep_v1_result = grape_sleep_v1(&sleep_v1_input);
            value["sleep_v1_input"] = serde_json::to_value(&sleep_v1_input).map_err(|error| {
                GrapeError::message(format!("cannot serialize sleep v1 input: {error}"))
            })?;
...
fn recovery_feature_score_bridge(args: RecoveryFeatureScoreArgs) -> GrapeResult<serde_json::Value> {
    validate_requested_primary_algorithm(
        "recovery",
        args.algorithm_id.as_deref(),
        args.algorithm_version.as_deref(),
        GRAPE_RECOVERY_V0_ID,
        GRAPE_RECOVERY_V0_VERSION,
    )?;
    let store = open_bridge_store(&args.database_path)?;
...
fn strain_feature_score_bridge(args: StrainFeatureScoreArgs) -> GrapeResult<serde_json::Value> {
    validate_requested_primary_algorithm(
        "strain",
        args.algorithm_id.as_deref(),
        args.algorithm_version.as_deref(),
        GRAPE_STRAIN_V0_ID,
        GRAPE_STRAIN_V0_VERSION,
    )?;
    let store = open_bridge_store(&args.database_path)?;
...
fn stress_feature_score_bridge(args: StressFeatureScoreArgs) -> GrapeResult<serde_json::Value> {
    validate_requested_primary_algorithm(
        "stress",
        args.algorithm_id.as_deref(),
        args.algorithm_version.as_deref(),
        GRAPE_STRESS_V0_ID,
        GRAPE_STRESS_V0_VERSION,
    )?;
    let store = open_bridge_store(&args.database_path)?;
    let hrv_start = args.hrv_start.as_deref().unwrap_or(&args.start);
    let hrv_end = args.hrv_end.as_deref().unwrap_or(&args.end);
...
fn health_sync_dry_run_bridge(input: HealthSyncDryRunInput) -> GrapeResult<serde_json::Value> {
...
fn capture_import_frame_batch_bridge(
    ...
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize capture import report: {error}"))
    })
}

fn overnight_mirror_batch_bridge(args: OvernightMirrorBatchArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(report)
        .map_err(|error| GrapeError::message(format!("cannot serialize overnight mirror: {error}")))
}
...
fn overnight_mirror_counts_bridge(
    ...
    serde_json::to_value(counts).map_err(|error| {
        GrapeError::message(format!("cannot serialize overnight mirror counts: {error}"))
    })
}

fn capture_timeline_bridge(args: CaptureTimelineArgs) -> GrapeResult<serde_json::Value> {
    if args.start.trim().is_empty() {
        return Err(GrapeError::message("start is required"));
    }
    if args.end.trim().is_empty() {
    ...
    serde_json::to_value(rows)
        .map_err(|error| GrapeError::message(format!("cannot serialize capture timeline: {error}")))
}
...
fn capture_observability_timeline_bridge(
    ...
    if args.start_unix_ms < 0 {
        return Err(GrapeError::message("start_unix_ms must be non-negative"));
    }
    if args.end_unix_ms <= 0 {
        return Err(GrapeError::message("end_unix_ms must be positive"));
    }
    ...
    serde_json::to_value(rows).map_err(|error| {
        GrapeError::message(format!("cannot serialize observability timeline: {error}"))
    })
}

fn capture_start_session_bridge(args: CaptureStartSessionArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let provenance_json = if args.provenance.is_null() {
        "{}".to_string()
    } else {
        if !args.provenance.is_object() {
            return Err(GrapeError::message("provenance must be a JSON object"));
        }
        serde_json::to_string(&args.provenance)
            .map_err(|error| GrapeError::message(format!("cannot serialize provenance: {error}")))?
    };
    ...
    let session = store.capture_session(&args.session_id)?.ok_or_else(|| {
        GrapeError::message(format!("capture session {} not found", args.session_id))
    })?;
    serde_json::to_value(json!({
        "schema": "grape.capture-session-result.v1",
        "inserted": inserted,
        "session": session,
    }))
    .map_err(|error| GrapeError::message(format!("cannot serialize capture session: {error}")))
}

fn capture_finish_session_bridge(args: CaptureFinishSessionArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let session =
...
fn capture_list_sessions_bridge(args: CaptureListSessionsArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let sessions = store.capture_sessions_between(args.start_unix_ms, args.end_unix_ms)?;
    serde_json::to_value(json!({
        "schema": "grape.capture-session-list.v1",
        "session_count": sessions.len(),
        "sessions": sessions,
    }))
    .map_err(|error| GrapeError::message(format!("cannot serialize capture session list: {error}")))
}
...
fn activity_create_session_bridge(
    ...
    let session = store.activity_session(&args.session_id)?.ok_or_else(|| {
        GrapeError::message(format!("activity session {} not found", args.session_id))
    })?;
    Ok(json!({
        "schema": "grape.activity-session-result.v1",
        "generated_by": "grape-bridge",
        "inserted": inserted,
        "session": session,
    }))
...
fn activity_get_session_bridge(args: ActivitySessionLookupArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let session = store.activity_session(&args.session_id)?.ok_or_else(|| {
        GrapeError::message(format!("activity session {} not found", args.session_id))
    })?;
...
fn activity_list_sessions_bridge(args: ActivitySessionListArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let sessions =
        store.activity_sessions_between(args.start_time_unix_ms, args.end_time_unix_ms)?;
    Ok(json!({
        "schema": "grape.activity-session-list.v1",
        "generated_by": "grape-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
        "session_count": sessions.len(),
...
fn activity_list_sessions_with_metrics_bridge(
    ...
    Ok(json!({
        "schema": "grape.activity-session-list-with-metrics.v1",
        "generated_by": "grape-bridge",
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
        "session_count": sessions.len(),
...
fn activity_correction_plans_bridge() -> GrapeResult<serde_json::Value> {
    let plans = activity_session_correction_plans();
    Ok(json!({
        "schema": "grape.activity-correction-plans.v1",
        "generated_by": "grape-bridge",
...
fn activity_apply_correction_bridge(
    ...
    if !args.details.is_object() {
        return Err(GrapeError::message("details must be a JSON object"));
    }
    if !args.provenance.is_object() {
        return Err(GrapeError::message("provenance must be a JSON object"));
    }
    ...
    match args.kind {
        ...
        ActivitySessionCorrectionKind::TrimStart => {
            start_time_unix_ms = args.start_time_unix_ms.ok_or_else(|| {
                GrapeError::message("start_time_unix_ms is required for trim_start corrections")
            })?;
        }
        ActivitySessionCorrectionKind::TrimEnd => {
            end_time_unix_ms = args.end_time_unix_ms.ok_or_else(|| {
                GrapeError::message("end_time_unix_ms is required for trim_end corrections")
            })?;
        }
    ...
    Ok(json!({
        "schema": "grape.activity-correction-result.v1",
        "generated_by": "grape-bridge",
        "session_id": args.session_id,
        "kind": args.kind,
        "updated": updated,
...
fn activity_delete_session_bridge(
    ...
    Ok(json!({
        "schema": "grape.activity-session-delete-result.v1",
        "generated_by": "grape-bridge",
        "session_id": args.session_id,
        "deleted": deleted,
    }))
...
fn activity_attach_metric_bridge(args: ActivityMetricAttachArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let provenance_json = json_object_string("provenance", &args.provenance)?;
    let quality_flags_json = serde_json::to_string(&args.quality_flags)
        .map_err(|error| GrapeError::message(format!("cannot serialize quality_flags: {error}")))?;
    ...
    let metric = store.activity_metric(&args.metric_id)?.ok_or_else(|| {
        GrapeError::message(format!("activity metric {} not found", args.metric_id))
    })?;
    Ok(json!({
        "schema": "grape.activity-metric-result.v1",
        "generated_by": "grape-bridge",
        "inserted": inserted,
        "metric": metric,
    }))
...
fn activity_attach_metrics_bridge(
        ...
        .map(|metric| {
            Ok(SerializedActivityMetricAttachArg {
                ...
                quality_flags_json: serde_json::to_string(&metric.quality_flags).map_err(
                    |error| GrapeError::message(format!("cannot serialize quality_flags: {error}")),
                )?,
                provenance_json: json_object_string("provenance", &metric.provenance)?,
            })
        })
        .collect::<GrapeResult<Vec<_>>>()?;
        ...
        args.metrics
            ...
            .map(|metric| {
                store.activity_metric(&metric.metric_id)?.ok_or_else(|| {
                    GrapeError::message(format!("activity metric {} not found", metric.metric_id))
                })
            })
            .collect::<GrapeResult<Vec<_>>>()?
    } else {
    ...
    Ok(json!({
        "schema": "grape.activity-metric-batch-result.v1",
        "generated_by": "grape-bridge",
...
fn activity_list_metrics_bridge(args: ActivityMetricListArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    let metrics = store.activity_metrics_for_session(&args.activity_session_id)?;
    Ok(json!({
        "schema": "grape.activity-metric-list.v1",
        "generated_by": "grape-bridge",
        "activity_session_id": args.activity_session_id,
        "metric_count": metrics.len(),
        "metrics": metrics,
...
fn activity_metrics_for_session_in_window_bridge(
    ...
    Ok(json!({
        "schema": "grape.activity-metric-window.v1",
        "generated_by": "grape-bridge",
        "activity_session_id": args.activity_session_id,
        "start_time_unix_ms": args.start_time_unix_ms,
        "end_time_unix_ms": args.end_time_unix_ms,
...
fn activity_attach_interval_bridge(
    ...
    let interval = store.activity_interval(&args.interval_id)?.ok_or_else(|| {
        GrapeError::message(format!("activity interval {} not found", args.interval_id))
    })?;
    Ok(json!({
        "schema": "grape.activity-interval-result.v1",
        "generated_by": "grape-bridge",
        "inserted": inserted,
        "interval": interval,
    }))
...
fn activity_list_intervals_bridge(
    ...
    Ok(json!({
        "schema": "grape.activity-interval-list.v1",
        "generated_by": "grape-bridge",
        "activity_session_id": args.activity_session_id,
...
fn external_sleep_history_import_bridge(
    ...
    Ok(json!({
        "schema": "grape.external-sleep-history-import-result.v1",
        "generated_by": "grape-bridge",
...
fn sleep_correction_label_bridge(args: SleepCorrectionLabelArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    let label = store
        ...
        .ok_or_else(|| GrapeError::message("sleep correction label was not stored"))?;
    Ok(json!({
        "schema": "grape.sleep-correction-label-result.v1",
        "generated_by": "grape-bridge",
        "inserted": inserted,
        "label": label,
...
fn sleep_correction_label_list_bridge(
    ...
    let distinct_sleep_window_sleep_id_count = labels
        ...
        .len();
    Ok(json!({
        "schema": "grape.sleep-correction-label-list.v1",
        "generated_by": "grape-bridge",
        "label_count": labels.len(),
...
fn sleep_v1_release_gate_bridge(args: SleepV1ReleaseGateArgs) -> GrapeResult<serde_json::Value> {
...
fn capture_correlation_bridge(args: CaptureCorrelationArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn capture_arrival_plan_bridge(args: CaptureArrivalPlanArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    let report = CaptureArrivalPlanReport {
        ...
        generated_by: "grape-capture-arrival-plan".to_string(),
        pass,
        start: args.start,
        end: args.end,
        min_owned_captures,
    ...
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize capture arrival plan: {error}"))
    })
}
...
fn capture_arrival_physical_rows(
    ...
    rows.push(capture_arrival_physical_row(
        ...
        "Record start, end, pauses, sport/activity type, confidence, and whether type came from WHOOP bytes, app metadata, or Grape inference.",
...
) -> GrapeResult<(Vec<CaptureSessionRow>, Vec<ActivitySessionRow>)> {
...
fn command_direct_send_gate_bridge(
    ...
    serde_json::to_value(gate)
        .map_err(|error| GrapeError::message(format!("cannot serialize command gate: {error}")))
}
...
fn command_capture_plan_bridge(args: CommandCapturePlanArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(report).map_err(|error| {
        GrapeError::message(format!("cannot serialize command capture plan: {error}"))
    })
}
...
fn command_import_validation_records_bridge(
    ...
    Ok(json!({
        "schema": "grape.command-validation-import-report.v1",
        "generated_by": "grape-command-validation-import",
    ...
    for result in results {
        store.upsert_command_validation_record(&CommandValidationRecord {
            ...
            report_json: serde_json::to_string(result).map_err(|error| {
                GrapeError::message(format!("cannot serialize command result: {error}"))
            })?,
        })?;
    }
    Ok(())
...
fn debug_start_session_bridge(args: DebugStartSessionArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(snapshot).map_err(|error| {
        GrapeError::message(format!("cannot serialize debug session snapshot: {error}"))
    })
}

fn debug_start_command_bridge(args: DebugStartCommandArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn debug_finish_command_bridge(args: DebugFinishCommandArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn debug_record_event_bridge(args: DebugRecordEventArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
    ...
    serde_json::to_value(event)
        .map_err(|error| GrapeError::message(format!("cannot serialize debug event: {error}")))
}

fn debug_session_snapshot_bridge(args: DebugSessionSnapshotArgs) -> GrapeResult<serde_json::Value> {
    let store = open_bridge_store(&args.database_path)?;
...
fn metric_result_to_value<T: Serialize>(result: T) -> GrapeResult<serde_json::Value> {
    serde_json::to_value(result)
        .map_err(|error| GrapeError::message(format!("cannot serialize metric result: {error}")))
}
...
) -> GrapeResult<Option<crate::store::CalibrationRunRecord>> {
...
fn open_bridge_store(database_path: &str) -> GrapeResult<GrapeStore> {
    if database_path.trim().is_empty() {
        return Err(GrapeError::message("database_path is required"));
    }
    GrapeStore::open(Path::new(database_path))
}

fn json_object_string(field_name: &str, value: &serde_json::Value) -> GrapeResult<String> {
    if !value.is_object() {
        return Err(GrapeError::message(format!(
    ...
    serde_json::to_string(value)
        .map_err(|error| GrapeError::message(format!("cannot serialize {field_name}: {error}")))
}

fn register_built_in_definitions(store: &GrapeStore) -> GrapeResult<()> {
    for definition in built_in_algorithm_definitions() {
        store.upsert_algorithm_definition(&definition)?;
    }
    Ok(())
...
fn request_args<T>(request: &BridgeRequest) -> GrapeResult<T>
where
    T: for<'de> Deserialize<'de>,
{
    serde_json::from_value(request.args.clone())
        .map_err(|error| GrapeError::message(format!("invalid args: {error}")))
}

fn parse_device_type(value: &str) -> GrapeResult<DeviceType> {
    match value {
        ...
        "GRAPE" | "Grape" | "grape" => Ok(DeviceType::Grape),
        other => Err(GrapeError::message(format!(
...
fn default_capture_sanitize_salt() -> String {
    "grape-capture-sanitize-v1".to_string()
}
...
mod tests {
    ...
    fn sleep_v1_external_history_prefers_detailed_stage_rows_over_empty_summary() {
        let store = GrapeStore::open_in_memory().unwrap();
