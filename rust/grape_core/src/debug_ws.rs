...
use crate::{
    ...
    store::{DebugCommandRow, DebugEventRow, DebugSessionRow, GrapeStore},
};

pub const DEBUG_WS_CONTRACT_SCHEMA: &str = "grape.debug-ws-contract.v1";
pub const DEBUG_EVENT_SCHEMA: &str = "grape.debug.event.v1";
pub const DEBUG_COMMAND_SCHEMA: &str = "grape.debug.command.v1";
...
pub fn validate_debug_ws_contract(input: &DebugWsContractInput) -> DebugWsContractReport {
    ...
    DebugWsContractReport {
        schema: "grape.debug-ws-contract-report.v1".to_string(),
        generated_by: "grape-debug-ws-contract".to_string(),
        pass,
        input_valid,
        bridge_valid,
        commands_valid,
...
) -> GrapeResult<DebugSessionSnapshot> {
...
pub fn start_debug_command(
    ...
    let args_json = serde_json::to_string(&input.command.args)
        .map_err(|error| GrapeError::message(format!("cannot serialize command args: {error}")))?;
...
pub fn finish_debug_command(
    ...
    let command = store.debug_command(&input.command_id)?.ok_or_else(|| {
        GrapeError::message(format!("debug command {} not found", input.command_id))
    })?;
...
pub fn append_debug_event(
...
) -> GrapeResult<DebugEventEnvelope> {
    validate_required("session_id", &input.session_id)?;
    if store.debug_session(&input.session_id)?.is_none() {
        return Err(GrapeError::message(format!(
            "debug session {} not found",
    ...
    if let Some(command_id) = input.command_id.as_deref() {
        if !command_id.trim().is_empty() {
            let command = store.debug_command(command_id)?.ok_or_else(|| {
                GrapeError::message(format!("debug command {command_id} not found"))
            })?;
            if command.session_id != input.session_id {
                return Err(GrapeError::message(format!(
    ...
    let data_json = serde_json::to_string(&event.data)
        .map_err(|error| GrapeError::message(format!("cannot serialize event data: {error}")))?;
...
pub fn debug_session_snapshot(
    ...
    let session = store
        ...
        .ok_or_else(|| GrapeError::message(format!("debug session {session_id} not found")))?;
    ...
    Ok(DebugSessionSnapshot {
        schema: "grape.debug-session-snapshot.v1".to_string(),
...
fn validate_bridge(bridge: &DebugBridgeConfig, issues: &mut Vec<String>) {
    ...
    if !bridge.url.contains("/grape-debug/stream") {
...
fn debug_ws_next_action(issue: &str) -> DebugWsNextAction {
    ...
    let action = match reason {
        "unsupported_schema" => {
            "Update the debug stream contract input to grape.debug-ws-contract.v1, then rerun the contract validator."
        }
        ...
        "bridge_url_missing_debug_stream_path" => {
            "Bind the debug stream on /grape-debug/stream so agents and tests connect to the expected path."
        }
        ...
        "command_schema_invalid" => {
            "Emit debug commands with schema grape.debug.command.v1, then rerun the contract validator."
        }
        "command_id_required" => {
        ...
        "event_schema_invalid" => {
            "Emit debug events with schema grape.debug.event.v1, then rerun the contract validator."
        }
        ...
        "event_sequence_not_strictly_increasing" => {
            "Append events through Grape storage so sequence numbers strictly increase before streaming."
        }
        ...
        _ => {
            "Resolve the debug stream contract issue, then rerun grape-debug-ws-contract or refresh the Debug snapshot."
        }
    };
    DebugWsNextAction {
        scope,
...
fn command_from_row(row: DebugCommandRow) -> GrapeResult<DebugCommandEnvelope> {
    let args = serde_json::from_str::<serde_json::Value>(&row.args_json)
        .map_err(|error| GrapeError::message(format!("cannot parse command args JSON: {error}")))?;
...
pub fn debug_event_envelope_from_row(row: DebugEventRow) -> GrapeResult<DebugEventEnvelope> {
    let data = serde_json::from_str::<serde_json::Value>(&row.data_json)
        .map_err(|error| GrapeError::message(format!("cannot parse event data JSON: {error}")))?;
...
fn object_value(value: serde_json::Value, name: &str) -> GrapeResult<serde_json::Value> {
    if value.is_object() {
        Ok(value)
    } else {
        Err(GrapeError::message(format!("{name} must be a JSON object")))
    }
}

fn u64_to_i64(name: &str, value: u64) -> GrapeResult<i64> {
    i64::try_from(value).map_err(|_| GrapeError::message(format!("{name} is too large")))
}

fn i64_to_u64(name: &str, value: i64) -> GrapeResult<u64> {
    u64::try_from(value).map_err(|_| GrapeError::message(format!("{name} is negative")))
}
