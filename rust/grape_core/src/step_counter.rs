...
use crate::{
    ...
    store::{
        DailyActivityMetricInput, GrapeStore, HourlyActivityMetricInput, MetricProvenanceInput,
        StepCounterSampleInput, StepCounterSampleRow,
    },
};

pub const STEP_COUNTER_INGEST_REPORT_SCHEMA: &str = "grape.step-counter-ingest-report.v1";
pub const STEP_COUNTER_DAILY_ROLLUP_REPORT_SCHEMA: &str =
    "grape.step-counter-daily-rollup-report.v1";
pub const STEP_COUNTER_HOURLY_ROLLUP_REPORT_SCHEMA: &str =
    "grape.step-counter-hourly-rollup-report.v1";
pub const ACTIVITY_UNAVAILABLE_DAILY_STATUS_REPORT_SCHEMA: &str =
    "grape.activity-unavailable-daily-status-report.v1";
pub const GRAPE_STEPS_DEVICE_COUNTER_V0_ID: &str = "grape.steps.device_counter.v0";
pub const GRAPE_STEPS_DEVICE_COUNTER_V0_VERSION: &str = "0.1.0";
pub const GRAPE_ACTIVITY_UNAVAILABLE_STATUS_V0_ID: &str = "grape.activity.unavailable_status.v0";
pub const GRAPE_ACTIVITY_UNAVAILABLE_STATUS_V0_VERSION: &str = "0.1.0";
...
) -> GrapeResult<StepCounterIngestReport> {
...
pub fn persist_step_counter_discovery(
        ...
        .filter(|candidate| candidate.match_kind == "step_count")
    {
        ...
        let provenance_json = json!({
            "algorithm": "grape.step_counter_ingest.v1",
    ...
    Ok(StepCounterIngestReport {
        ...
        generated_by: "grape-step-counter-ingest".to_string(),
...
pub fn rollup_device_step_counter_day(
...
) -> GrapeResult<StepCounterDailyRollupReport> {
    ...
    if pass && options.write_metric {
        ...
        let quality_flags_json = serde_json::to_string(&quality_flags).map_err(|error| {
            GrapeError::message(format!("cannot serialize quality flags: {error}"))
        })?;
        let provenance_json = json!({
            "algorithm": GRAPE_STEPS_DEVICE_COUNTER_V0_ID,
            "algorithm_version": GRAPE_STEPS_DEVICE_COUNTER_V0_VERSION,
            "source_kind": "device_counter",
    ...
    Ok(StepCounterDailyRollupReport {
        ...
        generated_by: "grape-step-counter-daily-rollup".to_string(),
        pass,
...
pub fn rollup_device_step_counter_hour(
...
) -> GrapeResult<StepCounterHourlyRollupReport> {
    ...
    Ok(StepCounterHourlyRollupReport {
        ...
        generated_by: "grape-step-counter-hourly-rollup".to_string(),
        pass,
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start_time_unix_ms: options.start_time_unix_ms,
...
pub fn rollup_activity_unavailable_daily_status_for_store(
...
) -> GrapeResult<ActivityUnavailableDailyStatusReport> {
    ...
    Ok(ActivityUnavailableDailyStatusReport {
        ...
        generated_by: "grape-activity-unavailable-daily-status".to_string(),
        pass: true,
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start_time_unix_ms: options.start_time_unix_ms,
...
fn activity_steps_unavailable_status_for_rollup(
    ...
    options: ActivityUnavailableDailyStatusOptions<'_>,
) -> GrapeResult<ActivityUnavailableMetricStatus> {
    ...
    if options.write_metric {
        ...
        let provenance_json = json!({
            "algorithm": GRAPE_ACTIVITY_UNAVAILABLE_STATUS_V0_ID,
            "algorithm_version": GRAPE_ACTIVITY_UNAVAILABLE_STATUS_V0_VERSION,
...
fn validate_rollup_options(options: &StepCounterDailyRollupOptions<'_>) -> GrapeResult<()> {
    ...
    if options.min_sample_count < 2 {
        return Err(GrapeError::message("min_sample_count must be at least 2"));
    }
    Ok(())
}

fn validate_hourly_rollup_options(options: &StepCounterHourlyRollupOptions<'_>) -> GrapeResult<()> {
    if options.date_key.trim().is_empty() {
        return Err(GrapeError::message("date_key is required"));
    }
    if options.timezone.trim().is_empty() {
