...
use crate::{
    ...
    metrics::{
        ...
        StressInput, grape_hrv_v0, grape_recovery_v0, grape_sleep_v0, grape_sleep_v1,
        grape_strain_v0, grape_stress_v0,
    },
    protocol::{DeviceType, FrameAccumulator, build_v5_payload_frame, parse_frame},
};

pub const PROPERTY_TEST_REPORT_SCHEMA: &str = "grape.property-test-report.v1";
...
pub fn run_property_suite(options: PropertySuiteOptions) -> GrapeResult<PropertySuiteReport> {
...
pub fn property_suite_report_from_groups(
    ...
    PropertySuiteReport {
        ...
        generated_by: "grape-property-test-suite".to_string(),
        seed,
        cases_per_group,
        pass,
        input_valid,
...
fn property_failure_action(group_name: &str, property: &str, reason: &str) -> &'static str {
    match reason {
        ...
        "algorithm_output_failure" => {
            "Fix the generated Grape score input or quality gate so valid generated inputs produce an output, then add a deterministic algorithm regression."
        }
        "algorithm_bounds_failure" => {
            "Clamp or correct the Grape score calculation so finite generated inputs stay within documented bounds and component counts remain consistent."
        }
        "algorithm_metamorphic_failure" => {
            "Review the Grape score formula for this monotonic relationship and add a hand-derived regression before changing expected behavior."
        }
        _ => {
...
fn parser_frame_properties(rng: &mut DeterministicRng, cases: usize) -> PropertyGroupReport {
    ...
    for case_index in 0..cases {
        ...
        let parsed_result =
            catch_unwind(AssertUnwindSafe(|| parse_frame(DeviceType::Grape, &frame)));
        match parsed_result {
            Ok(Ok(parsed)) => {
        ...
        if corrupted.len() > 12 {
            ...
            match catch_unwind(AssertUnwindSafe(|| {
                parse_frame(DeviceType::Grape, &corrupted)
            })) {
                Ok(Ok(parsed)) => {
                    group.check(
                        case_index,
        ...
        if catch_unwind(AssertUnwindSafe(|| parse_frame(DeviceType::Grape, &noise))).is_err() {
            group.fail(
                case_index,
...
fn algorithm_metamorphic_properties(cases: usize) -> PropertyGroupReport {
    ...
    for case_index in 0..cases {
        let sleep_short = grape_sleep_v0(&SleepInput {
        ...
        let sleep_long = grape_sleep_v0(&SleepInput {
        ...
        let sleep_v1_short = grape_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
                sleep_duration_minutes: 300.0,
        ...
        let sleep_v1_long = grape_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
                sleep_duration_minutes: 360.0,
                wake_after_sleep_onset_minutes: 30.0,
                wake_episode_count: 2,
        ...
        let sleep_v1_low_waso = grape_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
        ...
        .output
        .expect("valid sleep v1 input");
        let sleep_v1_high_waso = grape_sleep_v1(&SleepV1Input {
            sleep: SleepInput {
        ...
        let low_strain = grape_strain_v0(&StrainInput {
            hr_zone_minutes: vec![60.0, 0.0, 0.0, 0.0, 0.0],
            ..base_strain_input()
        })
        .output
        .expect("valid strain input");
        let high_strain = grape_strain_v0(&StrainInput {
        ...
        let recovery_low_hrv = grape_recovery_v0(&RecoveryInput {
            hrv_rmssd_ms: 40.0,
            ..base_recovery_input()
        })
        .output
        .expect("valid recovery input");
        let recovery_high_hrv = grape_recovery_v0(&RecoveryInput {
            hrv_rmssd_ms: 60.0,
            ..base_recovery_input()
        })
        .output
        ...
        let low_motion_stress = grape_stress_v0(&StressInput {
            motion_intensity_0_to_1: 0.0,
            ..base_stress_input()
        })
        .output
        .expect("valid stress input");
        let high_motion_stress = grape_stress_v0(&StressInput {
...
fn check_hrv_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    ...
    let result = grape_hrv_v0(&HrvInput {
        start_time: "2026-05-28T00:00:00Z".to_string(),
...
fn check_sleep_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    ...
    let result = grape_sleep_v0(&SleepInput {
        start_time: "2026-05-28T00:00:00Z".to_string(),
        end_time: "2026-05-28T08:00:00Z".to_string(),
...
fn check_sleep_v1_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    ...
    let result = grape_sleep_v1(&SleepV1Input {
        sleep: SleepInput {
        ...
        model_status: SleepModelStatusInput {
            ...
            trusted_grape_sleep_nights: rng.usize(12) as u32,
...
fn check_strain_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    ...
    let result = grape_strain_v0(&StrainInput {
...
fn check_recovery_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    let result = grape_recovery_v0(&RecoveryInput {
...
fn check_stress_bounds(group: &mut GroupBuilder, rng: &mut DeterministicRng, case_index: usize) {
    ...
    let result = grape_stress_v0(&StressInput {
        start_time: "2026-05-28T12:00:00Z".to_string(),
