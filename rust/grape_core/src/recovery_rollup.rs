...
use crate::{
    ...
    store::{DailyRecoveryMetricInput, GrapeStore, MetricProvenanceInput},
    validation_labels::{
...
pub const RESTING_HEART_RATE_DAILY_ROLLUP_REPORT_SCHEMA: &str =
    "grape.resting-heart-rate-daily-rollup-report.v1";
pub const RESTING_HEART_RATE_CAPTURE_VALIDATION_REPORT_SCHEMA: &str =
    "grape.resting-heart-rate-capture-validation-report.v1";
pub const RECOVERY_UNAVAILABLE_DAILY_STATUS_REPORT_SCHEMA: &str =
    "grape.recovery-unavailable-daily-status-report.v1";
pub const RECOVERY_SENSOR_DAILY_ROLLUP_REPORT_SCHEMA: &str =
    "grape.recovery-sensor-daily-rollup-report.v1";
pub const GRAPE_RESTING_HEART_RATE_DEVICE_SENSOR_V0_ID: &str =
    "grape.resting_heart_rate.device_sensor.v0";
pub const GRAPE_RESTING_HEART_RATE_DEVICE_SENSOR_V0_VERSION: &str = "0.1.0";
pub const GRAPE_RECOVERY_UNAVAILABLE_STATUS_V0_ID: &str = "grape.recovery.unavailable_status.v0";
pub const GRAPE_RECOVERY_UNAVAILABLE_STATUS_V0_VERSION: &str = "0.1.0";
pub const GRAPE_RECOVERY_SENSOR_DEVICE_SENSOR_V0_ID: &str =
    "grape.recovery_sensor.device_sensor.v0";
pub const GRAPE_RECOVERY_SENSOR_DEVICE_SENSOR_V0_VERSION: &str = "0.1.0";
...
pub fn rollup_resting_heart_rate_day_for_store(
...
) -> GrapeResult<RestingHeartRateDailyRollupReport> {
    validate_options(&options)?;
    let start_time_unix_ms = parse_rfc3339_utc_unix_ms(options.start)
        .ok_or_else(|| GrapeError::message("start must be an RFC3339 UTC timestamp"))?;
    let end_time_unix_ms = parse_rfc3339_utc_unix_ms(options.end)
        .ok_or_else(|| GrapeError::message("end must be an RFC3339 UTC timestamp"))?;
    if end_time_unix_ms <= start_time_unix_ms {
        return Err(GrapeError::message("end must be after start"));
    }
    ...
    if pass && options.write_metric {
        ...
        let quality_flags_json = serde_json::to_string(&quality_flags).map_err(|error| {
            GrapeError::message(format!("cannot serialize RHR quality flags: {error}"))
        })?;
        let provenance_json = json!({
            "algorithm": GRAPE_RESTING_HEART_RATE_DEVICE_SENSOR_V0_ID,
            "algorithm_version": GRAPE_RESTING_HEART_RATE_DEVICE_SENSOR_V0_VERSION,
    ...
    Ok(RestingHeartRateDailyRollupReport {
        ...
        generated_by: "grape-resting-heart-rate-daily-rollup".to_string(),
        pass,
...
pub fn validate_resting_heart_rate_capture_for_store(
...
) -> GrapeResult<RestingHeartRateCaptureValidationReport> {
    ...
    Ok(RestingHeartRateCaptureValidationReport {
        ...
        generated_by: "grape-resting-heart-rate-capture-validator".to_string(),
        ...
        algorithm_id: GRAPE_RESTING_HEART_RATE_DEVICE_SENSOR_V0_ID.to_string(),
        algorithm_version: GRAPE_RESTING_HEART_RATE_DEVICE_SENSOR_V0_VERSION.to_string(),
        resting_hr_rollup,
...
pub fn rollup_recovery_unavailable_daily_status_for_store(
...
) -> GrapeResult<RecoveryUnavailableDailyStatusReport> {
    ...
    Ok(RecoveryUnavailableDailyStatusReport {
        ...
        generated_by: "grape-recovery-unavailable-daily-status".to_string(),
        pass: issues.is_empty(),
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start: options.start.to_string(),
...
pub fn rollup_recovery_sensor_daily_for_store(
...
) -> GrapeResult<RecoverySensorDailyRollupReport> {
    validate_recovery_sensor_daily_options(&options)?;
    let start_time_unix_ms = parse_rfc3339_utc_unix_ms(options.start)
        .ok_or_else(|| GrapeError::message("start must be an RFC3339 UTC timestamp"))?;
    let end_time_unix_ms = parse_rfc3339_utc_unix_ms(options.end)
        .ok_or_else(|| GrapeError::message("end must be after start"))?;
    if end_time_unix_ms <= start_time_unix_ms {
        return Err(GrapeError::message("end must be after start"));
    }
    ...
    Ok(RecoverySensorDailyRollupReport {
        ...
        generated_by: "grape-recovery-sensor-daily-rollup".to_string(),
        pass: issues.is_empty(),
        date_key: options.date_key.to_string(),
        timezone: options.timezone.to_string(),
        start: options.start.to_string(),
...
fn recovery_unavailable_metric_status_for_widget(
...
) -> GrapeResult<RecoveryUnavailableMetricStatus> {
    ...
    if options.write_metric {
        ...
        let provenance_json = json!({
            "algorithm": GRAPE_RECOVERY_UNAVAILABLE_STATUS_V0_ID,
            "algorithm_version": GRAPE_RECOVERY_UNAVAILABLE_STATUS_V0_VERSION,
...
fn recovery_sensor_daily_metric_status_for_widget(
    ...
    start_time_unix_ms: i64,
    end_time_unix_ms: i64,
) -> GrapeResult<RecoverySensorDailyMetricStatus> {
    let metric_name = recovery_metric_name(&widget.metric_id).to_string();
    ...
    if options.write_metric && promotable_value {
        ...
        let provenance_json = json!({
            "algorithm": GRAPE_RECOVERY_SENSOR_DEVICE_SENSOR_V0_ID,
            "algorithm_version": GRAPE_RECOVERY_SENSOR_DEVICE_SENSOR_V0_VERSION,
            "source_kind": "device_sensor",
            "metric_id": widget.metric_id,
            "metric_name": metric_name,
            "date_key": options.date_key,
...
fn validate_options(options: &RestingHeartRateDailyRollupOptions<'_>) -> GrapeResult<()> {
    if options.date_key.trim().is_empty() {
        return Err(GrapeError::message("date_key is required"));
    }
    if options.timezone.trim().is_empty() {
        return Err(GrapeError::message("timezone is required"));
    }
    ...
    if options.min_sample_count == 0 {
        return Err(GrapeError::message("min_sample_count must be at least 1"));
    }
    Ok(())
}
...
fn validate_rhr_validation_options(
    ...
    if !options.tolerance_bpm.is_finite() || options.tolerance_bpm < 0.0 {
        return Err(GrapeError::message("tolerance_bpm must be nonnegative"));
    }
