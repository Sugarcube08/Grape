...
use crate::{
    ...
    metrics::{
        GRAPE_HRV_V0_ID, GRAPE_HRV_V0_VERSION, GRAPE_SLEEP_V0_ID, GRAPE_SLEEP_V0_VERSION,
        GRAPE_SLEEP_V1_ID, GRAPE_SLEEP_V1_VERSION, GRAPE_STRAIN_V0_ID, GRAPE_STRAIN_V0_VERSION,
        GRAPE_STRESS_V0_ID, GRAPE_STRESS_V0_VERSION, HrvInput, SleepInput, SleepV1Input,
        StrainInput, StressInput, grape_hrv_v0, grape_sleep_v0, grape_sleep_v1, grape_strain_v0,
        grape_stress_v0,
    },
    reference::{
...
pub const ALGORITHM_COMPARISON_SCHEMA: &str = "grape.algorithm-comparison-report.v1";
...
pub struct AlgorithmComparisonReport {
    ...
    pub grape_output_ready: bool,
    pub reference_output_ready: bool,
    pub shared_fields_ready: bool,
    pub pass: bool,
    pub grape_algorithm_id: String,
    pub grape_algorithm_version: String,
    ...
    pub grape_output: Option<serde_json::Value>,
    pub reference_output: Option<serde_json::Value>,
    pub grape_quality_flags: Vec<String>,
...
pub fn compare_hrv_grape_to_reference(input: &HrvInput) -> GrapeResult<AlgorithmComparisonReport> {
    let grape = grape_hrv_v0(input);
    let reference = reference_hrv_time_domain(input);
    let mut deltas = Vec::new();
    let mut quality_flags = Vec::new();
    let mut errors = prefixed_errors("grape", &grape.errors);
    errors.extend(prefixed_errors("reference", &reference.errors));

    if let (Some(grape_output), Some(reference_output)) = (&grape.output, &reference.output) {
        push_delta(
            &mut deltas,
            "mean_nn_ms",
            "grape_output.mean_nn_ms",
    ...
    comparison_report(ComparisonParts {
        family: "hrv",
        grape_algorithm_id: GRAPE_HRV_V0_ID,
        grape_algorithm_version: GRAPE_HRV_V0_VERSION,
        ...
        grape_output: serialize_optional("grape HRV output", &grape.output)?,
        reference_output: serialize_optional("reference HRV output", &reference.output)?,
        grape_quality_flags: grape.quality_flags,
...
pub fn compare_sleep_grape_to_reference(
    input: &SleepInput,
) -> GrapeResult<AlgorithmComparisonReport> {
    let grape = grape_sleep_v0(input);
    ...
    if let (Some(grape_output), Some(reference_output)) = (&grape.output, &reference.output) {
        push_delta(
            &mut deltas,
            "time_in_bed_minutes",
            "grape_input.time_in_bed_minutes",
        ...
        push_delta(
            &mut deltas,
            "sleep_minutes",
            "grape_input.sleep_duration_minutes",
        ...
        push_delta(
            &mut deltas,
            "wake_minutes",
            "grape_input.time_in_bed_minutes - grape_input.sleep_duration_minutes",
        ...
        push_delta(
            ...
            "grape_output.efficiency_fraction",
        ...
        push_delta(
            ...
            "grape_input.disturbance_count / grape_input.sleep_duration_hours",
    ...
    comparison_report(ComparisonParts {
        family: "sleep",
        grape_algorithm_id: GRAPE_SLEEP_V0_ID,
        grape_algorithm_version: GRAPE_SLEEP_V0_VERSION,
        ...
        non_comparable_fields: vec![
            "grape_output.score_0_to_100 has no benchmark-only actigraphy score equivalent"
                .to_string(),
            "grape_output.sleep_debt_minutes depends on sleep need, not just the actigraphy window"
                .to_string(),
            "grape_input.midpoint_deviation_minutes is a Grape consistency input with no internal actigraphy-summary equivalent"
                .to_string(),
        ],
        grape_output: serialize_optional("grape sleep output", &grape.output)?,
...
pub fn compare_sleep_v1_grape_to_reference(
    input: &SleepV1Input,
) -> GrapeResult<AlgorithmComparisonReport> {
    let grape = grape_sleep_v1(input);
    ...
    if let (Some(grape_output), Some(reference_output)) = (&grape.output, &reference.output) {
        push_delta(
            &mut deltas,
            "time_in_bed_minutes",
            "grape_output.time_in_bed_minutes",
            "reference_output.time_in_bed_minutes",
            "minutes",
            grape_output.time_in_bed_minutes,
            reference_output.time_in_bed_minutes,
        ...
        push_delta(
            &mut deltas,
            "sleep_minutes",
            "grape_output.sleep_duration_minutes",
            "reference_output.sleep_minutes",
            "minutes",
            grape_output.sleep_duration_minutes,
            reference_output.sleep_minutes,
        ...
        push_delta(
            &mut deltas,
            "sleep_efficiency_fraction",
            "grape_output.sleep_efficiency_fraction",
            "reference_output.sleep_efficiency_fraction",
            "fraction",
            grape_output.sleep_efficiency_fraction,
            reference_output.sleep_efficiency_fraction,
        ...
        push_delta(
            &mut deltas,
            "wake_after_sleep_onset_minutes",
            "grape_output.wake_after_sleep_onset_minutes",
            "reference_output.wake_after_sleep_onset_minutes",
            "minutes",
            grape_output.wake_after_sleep_onset_minutes,
            reference_output.wake_after_sleep_onset_minutes,
        ...
        push_delta(
            &mut deltas,
            "fragmentation_index_per_hour",
            "grape_input.disturbance_count / grape_output.sleep_duration_hours",
            "reference_output.fragmentation_index_per_hour",
            "events_per_hour",
            fragmentation_index_per_hour(
    ...
    let mut report = comparison_report(ComparisonParts {
        family: "sleep",
        grape_algorithm_id: GRAPE_SLEEP_V1_ID,
        grape_algorithm_version: GRAPE_SLEEP_V1_VERSION,
        reference_algorithm_id: REFERENCE_SLEEP_ACTIGRAPHY_ID,
        reference_algorithm_version: REFERENCE_SLEEP_ACTIGRAPHY_VERSION,
        ...
        non_comparable_fields: vec![
            "grape_output.score_0_to_100 has no benchmark-only actigraphy score equivalent"
                .to_string(),
            "grape_output.rolling_sleep_debt_minutes depends on prior nights and sleep need"
                .to_string(),
            "grape_output.model_status has no benchmark-only actigraphy equivalent".to_string(),
            "grape_output.stage_segments are heuristic and require label calibration".to_string(),
        ],
        grape_output: serialize_optional("grape sleep v1 output", &grape.output)?,
        reference_output: serialize_optional("reference sleep output", &reference.output)?,
        grape_quality_flags: grape.quality_flags,
        reference_quality_flags: reference.quality_flags,
        quality_flags,
        ...
        provenance: json!({
            ...
            "grape_comparable_inputs": {
                "disturbance_count": input.sleep.disturbance_count,
                "fragmentation_index_per_hour": grape
                    .output
                    .as_ref()
...
pub fn compare_sleep_v1_grape_to_external_reference_report(
    input: &SleepV1Input,
    ...
    if let (Some(grape_output), Some(_reference_output)) = (&grape.output, &reference.output) {
    ...
    non_comparable_fields.extend([
        "grape_output.score_0_to_100 has no external actigraphy summary score equivalent"
            .to_string(),
        "grape_output.rolling_sleep_debt_minutes depends on prior nights and sleep need"
            .to_string(),
        "grape_output.model_status has no external actigraphy equivalent".to_string(),
        "grape_output.stage_segments are heuristic and require label calibration".to_string(),
    ]);
...
pub fn compare_sleep_grape_to_external_reference_report(
    input: &SleepInput,
    reference_report: &serde_json::Value,
) -> GrapeResult<AlgorithmComparisonReport> {
    let reference = ExternalReferenceReport::from_json(reference_report)?;
    ...
    non_comparable_fields.extend([
        "grape_output.score_0_to_100 has no external actigraphy summary score equivalent"
            .to_string(),
        "grape_output.sleep_debt_minutes depends on sleep need, not just the external actigraphy window"
            .to_string(),
        "grape_input.midpoint_deviation_minutes is a Grape consistency input with no external actigraphy-summary equivalent"
            .to_string(),
    ]);
...
pub fn compare_strain_grape_to_reference(
    input: &StrainInput,
) -> GrapeResult<AlgorithmComparisonReport> {
    let grape = grape_strain_v0(input);
    ...
    comparison_report(ComparisonParts {
        family: "strain",
        grape_algorithm_id: GRAPE_STRAIN_V0_ID,
        grape_algorithm_version: GRAPE_STRAIN_V0_VERSION,
        ...
        deltas,
        non_comparable_fields: vec![
            "grape_output.score_0_to_21 has no Edwards-zone-load score equivalent".to_string(),
            "grape_output.average_hr_reserve_fraction is not part of Edwards zone load".to_string(),
            "reference_output.edwards_load_per_hour is not emitted by Grape strain v0".to_string(),
        ],
        grape_output: serialize_optional("grape strain output", &grape.output)?,
...
pub fn compare_stress_grape_to_reference(
    input: &StressInput,
) -> GrapeResult<AlgorithmComparisonReport> {
    let grape = grape_stress_v0(input);
    ...
    if let (Some(grape_output), Some(reference_output)) = (&grape.output, &reference.output) {
        push_delta(
            ...
            "grape_output.heart_rate_elevation_score",
        ...
        push_delta(
            ...
            "grape_output.hrv_suppression_score",
    ...
    comparison_report(ComparisonParts {
        family: "stress",
        grape_algorithm_id: GRAPE_STRESS_V0_ID,
        grape_algorithm_version: GRAPE_STRESS_V0_VERSION,
        ...
        deltas,
        non_comparable_fields: vec![
            "grape_output.score_0_to_100 includes motion adjustment while the reference proxy is unadjusted".to_string(),
            "grape_output.motion_adjusted_hr_score has no reference proxy equivalent".to_string(),
            "reference_output.unadjusted_stress_score_0_to_100 ignores motion context".to_string(),
        ],
        grape_output: serialize_optional("grape stress output", &grape.output)?,
...
struct ComparisonParts<'a> {
    family: &'a str,
    grape_algorithm_id: &'a str,
    grape_algorithm_version: &'a str,
    ...
    grape_output: Option<serde_json::Value>,
    reference_output: Option<serde_json::Value>,
    grape_quality_flags: Vec<String>,
...
impl ExternalReferenceReport {
    fn from_json(value: &serde_json::Value) -> GrapeResult<Self> {
        let mut report: ExternalReferenceReport =
            serde_json::from_value(value.clone()).map_err(|error| {
                GrapeError::message(format!("invalid external reference report: {error}"))
            })?;
        if !matches!(
            report.schema.as_str(),
            "grape.reference-algo-report.v1" | "grape.external-reference-output.v1"
        ) {
            return Err(GrapeError::message(format!(
    ...
    fn contract_errors(&self) -> Vec<String> {
        ...
        match self.schema.as_str() {
            "grape.external-reference-output.v1" => {
            ...
            "grape.reference-algo-report.v1" => {
                if self
                    .provenance
                    .get("provider_kind")
...
fn comparison_report(parts: ComparisonParts<'_>) -> GrapeResult<AlgorithmComparisonReport> {
    ...
    let grape_output_ready = parts.grape_output.is_some();
    ...
    let data_coverage = comparison_data_coverage(
        parts.family,
        parts.grape_algorithm_id,
        parts.grape_output.as_ref(),
    );

    Ok(AlgorithmComparisonReport {
        schema: ALGORITHM_COMPARISON_SCHEMA.to_string(),
        generated_by: "grape.algorithm_compare".to_string(),
        ...
        grape_algorithm_id: parts.grape_algorithm_id.to_string(),
        grape_algorithm_version: parts.grape_algorithm_version.to_string(),
        ...
        grape_output: parts.grape_output,
        reference_output: parts.reference_output,
        grape_quality_flags: parts.grape_quality_flags,
...
pub(crate) fn sleep_v1_benchmark_acceptance_summary(report: &AlgorithmComparisonReport) -> Value {
    let coverage = report
        .data_coverage
        .as_ref()
        .and_then(|coverage| coverage.get("grape_output_data_coverage_fraction"))
    ...
    json!({
        ...
        "grape_output_ready": report.grape_output_ready,
        "reference_output_ready": report.reference_output_ready,
        "shared_fields_ready": report.shared_fields_ready,
        "grape_algorithm_id": report.grape_algorithm_id,
        "grape_algorithm_version": report.grape_algorithm_version,
        ...
        "grape_quality_flag_count": report.grape_quality_flags.len(),
...
fn comparison_data_coverage(
    family: &str,
    grape_algorithm_id: &str,
    grape_output: Option<&serde_json::Value>,
) -> Option<serde_json::Value> {
    if family != "sleep" || grape_algorithm_id != GRAPE_SLEEP_V1_ID {
        return None;
    }
    let coverage = grape_output?
    ...
    Some(json!({
        "grape_output_data_coverage_fraction": coverage,
    }))
}
...
fn push_delta(
    ...
    let absolute_delta = grape_value - reference_value;
    ...
    deltas.push(AlgorithmComparisonDelta {
        ...
        grape_path: grape_path.to_string(),
...
fn serialize_optional<T: Serialize>(
    label: &str,
    output: &Option<T>,
) -> GrapeResult<Option<serde_json::Value>> {
    output
        .as_ref()
        .map(serde_json::to_value)
        .transpose()
        .map_err(|error| GrapeError::message(format!("cannot serialize {label}: {error}")))
}
...
fn algorithm_comparison_quality_action(flag: &str) -> AlgorithmComparisonNextAction {
    match flag {
        "comparison_outputs_missing" => AlgorithmComparisonNextAction {
            ...
            action: "Fix Grape/reference input requirements so both algorithms emit outputs before comparing shared fields.".to_string(),
        },
...
fn algorithm_comparison_error_action(error: &str) -> AlgorithmComparisonNextAction {
    if let Some(field) = error.strip_prefix("non_finite_delta:") {
        AlgorithmComparisonNextAction {
            ...
            action: "Check the Grape and reference outputs for non-finite values before trusting this delta.".to_string(),
        }
        ...
        AlgorithmComparisonNextAction {
            ...
            action: format!(
                "Regenerate the reference report through grape-reference-algo-runner or a validated adapter so provider metadata, output units, and provenance satisfy the benchmark contract; issue `{error}`."
            ),
        }
    } else if let Some(error) = error.strip_prefix("grape:") {
        AlgorithmComparisonNextAction {
            scope: "grape".to_string(),
            reason: "grape_algorithm_error".to_string(),
            action: format!(
                "Fix the Grape algorithm input or implementation error `{error}` before using this comparison."
            ),
        }
