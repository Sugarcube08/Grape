...
use crate::{
    ...
    store::{
        ...
        GrapeStore, RawEvidenceInput, RawEvidencePayloadRetentionReport,
    },
...
) -> GrapeResult<CapturedFrameBatchImportReport> {
...
fn import_captured_frame_batch_with_output_options_in_transaction(
    ...
    Ok(CapturedFrameBatchImportReport {
        schema: "grape.captured-frame-batch-import-report.v1".to_string(),
        generated_by: "grape-capture-import".to_string(),
...
pub fn import_capture_sqlite(
...
) -> GrapeResult<CaptureSqliteImportReport> {
    let mut issues = Vec::new();
    ...
    Ok(CaptureSqliteImportReport {
        schema: "grape.capture-sqlite-import-report.v1".to_string(),
        generated_by: "grape-capture-sqlite-import".to_string(),
        source_database_path: options.source_database_path.display().to_string(),
...
pub fn import_fixture_index(
    ...
    CaptureImportReport {
        schema: "grape.capture-import-report.v1".to_string(),
        generated_by: "grape-capture-import".to_string(),
...
fn import_captured_frame(
    ...
    parser_version: &str,
) -> GrapeResult<CapturedFrameImportResult> {
    let mut timing = CapturedFrameBatchTimingAccumulator::default();
...
fn load_capture_sqlite_frame_rows(path: &Path) -> GrapeResult<Vec<CaptureSqliteFrameRow>> {
    ...
    let mut statement = connection
        ...
        .map_err(|error| GrapeError::message(format!("cannot query capture sqlite: {error}")))?;
    let rows = statement
        .query_map([], |row| {
        ...
        .map_err(|error| GrapeError::message(format!("cannot scan capture sqlite: {error}")))?;

    rows.collect::<Result<Vec<_>, _>>()
        .map_err(GrapeError::from)
}

fn capture_sqlite_table_exists(connection: &Connection, table: &str) -> GrapeResult<bool> {
    let count: i64 = connection
        ...
        .map_err(|error| GrapeError::message(format!("cannot inspect capture sqlite: {error}")))?;
    Ok(count > 0)
}
...
fn capture_import_issue_action(issue: &str) -> (&'static str, &'static str) {
        ...
        || issue.starts_with("cannot read captured frame batch")
    {
        ...
        (
            ...
            "Regenerate the captured-frame batch with the supported grape.captured-frame-batch schema before importing it.",
        )
...
pub fn ensure_database_parent(path: &Path) -> GrapeResult<()> {
